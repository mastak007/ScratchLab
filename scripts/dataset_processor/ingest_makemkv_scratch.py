...
SOURCE_TYPE = "makemkv_dvd_rip"
SOURCE_COLLECTION = "qbert_makemkv_dvd"
DEFAULT_PERFORMER = "Qbert"
SUPPORTED_SUFFIX = ".mkv"
IGNORED_GENERATED_SUFFIXES = (
    "_performance.mkv",
    "_performance_clean.mkv",
    "_instruction.mkv",
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest MakeMKV scratch folders into canonical per-angle video and per-stream audio outputs."
    )
    parser.add_argument("--input-root", required=True, help="Root directory containing MakeMKV scratch folders.")
    ...
    parser.add_argument("--audio-map", help="Optional JSON file mapping MKV audio stream indexes to stream roles.")
    parser.add_argument("--inspect-streams", action="store_true", help="Inspect MKV stream roles without writing output.")
...
def build_audio_metadata(
    *,
    ...
    return {
        ...
        "labelSource": "folder_name_makemkv_ingest",
        "confidence": 0.9,
        "notes": "",
    }


class MakeMKVScratchIngester:
    ...
    def discover_source_folders(self) -> dict[Path, list[Path]]:
        ...
        *,
        ...
        for source_clip in sorted(source_clips, key=lambda clip: clip.camera_angle or clip.source_path.name.lower()):
            ...
            video_plan = VideoPlan(
                ...
                output_filename=f"{camera_angle}_video.mkv",
            )
    ...
    def run(self) -> int:
        ...
        source_folders = self.discover_source_folders()
...
def main() -> int:
    try:
        args = parse_args()
        return MakeMKVScratchIngester(args).run()
    except RuntimeError as error:
        print(str(error), file=sys.stderr)
        return 1
