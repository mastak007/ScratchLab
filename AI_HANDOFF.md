# Current CXL diagnostic update installed — 2026-09-12 21:19 NZST

Work only in `/Users/karlwatson/Developer/ScratchLab-CXL-Recovery-20260912/source`, branch `codex/cxl-capture-recovery-20260912`. User authorized commits and pushes as needed; a coherent67-file checkpoint is being recorded on this branch and pushed normally to the verified origin. Inspect git log/remote and ../evidence/cxl-output-meter-20260912/checkpoint.json for the final receipt. Do not merge into the separately dirty primary. Pilot audio and the empty test plist remain untracked intentionally.

Installed app: `/Users/karlwatson/Applications/ScratchLab CXL.app`, SHA256 `19f0fea027a5503c31e93c290497c6ea649264690967806bfbaea4a1ff2b4380`, observed running PID47873. Previous app retained at `/Users/karlwatson/Applications/ScratchLab CXL.previous-20260912-211841.app`. App was confirmed closed by process inspection before replacement. Sky still reports native-pipe failure; exact executable runs, but no visual inspection or hardware acceptance is claimed.

Actual AHHH PCM peak meter/direct verified RaneONE USB3/4 routing implemented. UID intent survives automatic disconnect fallback; unvalidated outputs fail loudly; recording checks actual running route; Mac monitor optional/off by default and explicitly delayed. Output/input provenance remain distinct and preparation-time audit is labeled accurately.271 unique focused tests passed,2 skipped; universal Release/signature passed;462 frozen inputs unchanged. Full all-platform gate deferred.

Next: one Movement check(no beat) on Rane ONE through Rane headphones/speakers, one slow scratch, Stop/Finalize on Mac, Play whole take, Save Capture, inspect export and Watch Stop. Beat/count-in still use independent system-default output. Physical mixer gain parity and Seventy-Two/Twelve input/output mapping remain unverified; do not send this as production-ready CXL reference capture. No further coding without concrete new evidence. Full details ../evidence/cxl-output-meter-20260912/RESULT.md. Earlier statuses below are historical.

# Current audio fix: tests passed, Release build running — 2026-09-12

Work only in Recovery/source. New branch `codex/cxl-capture-recovery-20260912`; origin restored from primary checkout's verified GitHub URL. User explicitly authorizes commits and pushes as needed. No checkpoint committed or pushed yet. Prepare the coherent67-file source/test/documentation checkpoint; exclude pilot audio, empty test plist, builds, evidence/signing data and user captures. Primary Downloads checkout remains untouched.

Current focused result:273 unique tests,271 passed/2 skipped/0 failed across both configurations (546 executions). Skips: debug capture env flag absent; test host supplied no main-mixer capture callbacks. Initial compile failure in optional CoreAudio pointer fixed and retained. All462inputs match final tested manifest. Universal Release build is running; do not launch concurrent xcodebuild. Source fixes include actual AHHH peak meter, direct verified RaneONE USB3/4, optional delayed Macmonitor, UID-stable no-fallback on disconnect, live route verification and preparation-time route audit. Independent final correction review found no remaining blocker in this scope.

Installed app remains prior SHA55cc6e...; no new install or physical acceptance. Finish Release/sign/stage then get fresh operator stopped/closed confirmation before replacement. Beats/count-in remain system-default; production72/Twelve and one full no-beat capture/playback/export test are still outstanding. Evidence ../evidence/cxl-output-meter-20260912. Older 'no commit/push' instructions below are superseded by current explicit user authorization.

# Active CXL output-meter and routing correction — 2026-09-12

Work ONLY in `/Users/karlwatson/Developer/ScratchLab-CXL-Recovery-20260912/source`. Preserve all inherited dirty work; no commits/pushes. User reports CXL input43% before AHHH begins and physical Rane LEDs lag sound heard from Mac speakers/wired Mac headphones; sound first, LEDs later. User asks to get the complete path right before another test.

Confirmed code causes: main CXL meter reads all-channel Rane input RMS*10 with smoothing, not internal AHHH output; primary playback goes direct to default Mac while Rane receives a buffered asynchronous copy, and active-output label misrepresents that mirror as primary. Actual standalone WAV captures internal mainMixer output. Do not claim measured physical delay or identify the non-AHHH input signal without evidence.

In progress: cxl_continuous_preview_fix owns controller/engine permanent-tap actual peak telemetry and tests; root owns ReferenceAuthoringView actual digital dBFS meter/UI/tests. cxl_camera_boxes completed read-only routing plan and may implement after meter worker stable: Rane primary with verified USB3/4 for supported ONE, honest optional delayed Mac monitor, capture-safe rebind, route failures/status. screenshot_attachment_diagnosis is auditing separate beat output. No current slice build/test/install yet. Evidence ../evidence/cxl-output-meter-20260912. See agent messages and scope.md.

Installed app remains previous live/framing build at ~/Applications/ScratchLab CXL.app, SHA55cc6e408500fe194aa9bc46ac11ff41d90333f1cdbafcafb246f4f373d63fea; last PID39632. Previous stopped confirmation predates current launch: finish tested candidate first, then establish current stopped/closed status before replacement. Sky helper native-pipe failure prevents UI inspection. Do not reset permissions or restart Codex during work. Actual hardware timing and production Rane72/Twelve remain unverified. Below completed-slice notes are historical.

# Current CXL update installed — awaiting one live movement check (2026-09-12 20:50 NZST)

Operator confirmed CXL closed and Watch stopped. Installed current candidate at `/Users/karlwatson/Applications/ScratchLab CXL.app`; exact SHA55cc6e408500fe194aa9bc46ac11ff41d90333f1cdbafcafb246f4f373d63fea verified running as PID39632 after33seconds. Rollback `/Users/karlwatson/Applications/ScratchLab CXL.previous-20260912-204914.app`. New app launched through Sky exact path; helper still fails UI capture, so no visual/hardware claim. No new recording started.

Next bounded operator check: one slow right-platter movement; inspect AHHH waveform/cursor below camera, continuous physical trace, and full-frame left/wide-mixer/right camera boxes. All software fixes and targeted checks complete; do not rebuild unchanged code. Current source/evidence remain Recovery/source and ../evidence/cxl-live-framing-20260912/RESULT.md. 358 unique tests passed,1optional artifact skip; universal Release/signature/source hashes verified. Full gate deferred; companions unchanged. All earlier pending installation notes below are historical and superseded. No commit/push.

# CXL live/framing software ready — awaiting safe replacement (2026-09-12 20:47 NZST)

Source: `/Users/karlwatson/Developer/ScratchLab-CXL-Recovery-20260912/source`. 358 unique tests passed, 1 historical take003 skip, universal Release passed. Actual renderer AHHH cursor, continuous CXL motion for all techniques, full-frame33/34/33camera boxes with separate persistence and take-start audit all implemented. Resize review blockers fixed. Candidate: `../evidence/cxl-live-framing-20260912/staged/ScratchLab CXL.app`, SHA55cc6e408500fe194aa9bc46ac11ff41d90333f1cdbafcafb246f4f373d63fea. Signed ad hoc, strict/deep verified, original bundle identity/permissions preserved;462source hashes unchanged. Full gate deferred; no current hardware acceptance.

Pending user question asked after build: save take, stop Watch if running and quit CXL; confirm closed/stopped. Do not replace while app may capture. Installed app remains prior hash5d0dd754ded00d67adb6b31f7a8e6d5bb5176a245b9982bf394fbf6f1599b0a4, last PID29616. Codex Sky helper continues native pipe failure, cannot safely inspect/quit via UI. Once status permits, preserve rollback app, install staged app into `/Users/karlwatson/Applications/ScratchLab CXL.app`, launch exact app through supported tool (may fail; verify PID), and report truthfully. Do not restart Codex or alter permissions while coding. Read RESULT.md for exact test accounting and13-file changes. No commits/pushes.

# Active CXL live/framing repair — 2026-09-12

Work only in `/Users/karlwatson/Developer/ScratchLab-CXL-Recovery-20260912/source`; primary Downloads checkout is untouched. Branch `codex/cxl-auto-p21-final-gates`, inherited dirty work preserved. No commit/push.

Current user scope: fix disconnected unequal live push/pull strokes, show local AHHH waveform and playhead, and camera calibration guides auto-fill left/right with mixer twice ordinary width. User confirmed camera boxes. Prior assumption of external Serato was incorrect: standalone internal sample was already the selected product path.

In progress: every CXL technique uses continuous canonical projection with observed tracker (Practice unchanged); CXL camera component has real image bounds, separate persisted33/34/33 manual guides, lock/edit protection and tracking ROI; existing waveform renderer reused under video, pending new coherent actual renderer cursor telemetry. Root owns ReferenceAuthoringView/MacAnalyzerView and docs. Camera worker owns camera/overlay/engine guide changes; cursor worker owns renderer/controller telemetry. Focused tests and universal Release build have NOT run yet for this slice. Current installed app still has prior hash5d0dd754ded00d67adb6b31f7a8e6d5bb5176a245b9982bf394fbf6f1599b0a4.

Evidence root `../evidence/cxl-live-framing-20260912`; inherited-diff.patch is a mid-work snapshot, explicitly not pre-turn. Previous shipped inputs remain in record-start-recovery-20260912. Capture data untouched. Do not replace/restart app while user might be recording; finish build then establish stopped status.

Codex screenshot shortcut failure is independently diagnosed: helper SIGTRAP/Array.remove(at:) crash; hotkey selected CXL but hadScreenshot=false. App control has same native-pipe failure. Manual saved JPEG attachment works. Do not restart Codex during coding. No permission denial established.

# Current CXL capture/review update installed — 2026-09-12

Work only in /Users/karlwatson/Developer/ScratchLab-CXL-Recovery-20260912/source; branch codex/cxl-auto-p21-final-gates. All inherited dirty changes preserved, primary Downloads/ScratchLab untouched, no commit or push.

Installed and running: /Users/karlwatson/Applications/ScratchLab CXL.app, executable SHA256 5d0dd754ded00d67adb6b31f7a8e6d5bb5176a245b9982bf394fbf6f1599b0a4, PID29616. Universal arm64/x86_64, strict deep signature verified, bundle ID/permission descriptions/entitlements preserved. Prior hardware app retained at /Users/karlwatson/Applications/ScratchLab CXL.previous-20260912-200414.app. User explicitly confirmed Watch stopped and CXL closed before update. CUA getApp again reported native pipe closed, but exact new executable remained running22seconds afterward. Window controls not inspected; do not claim UI/hardware acceptance.

Updated signed iPhone and embedded Watch builds passed, installed in place on physical paired iPhone K and Watch Karl Watson, then both launched using devicectl. Existing bundle IDs retained, no uninstall or data deletion. Exact receipts and source-product hashes in ../evidence/record-start-recovery-20260912/. Watch retry/identity changes require these installed companion versions. No fresh recording, source-channel proof, motion continuity, Stop transfer, approval or production72/Twelve acceptance was performed by agent.

Implemented: any known starting fader position and fresh quiet audio allow diagnostic recording; missing Watch is advisory; optional Watch handshake remains bounded and exact-take cleanup preserves newer ownership. Freeze matching calibration/source at Record, including parked real raw127 evidence. Explicit Movement check(no beat), zero count-in and repetitions, manual stop, actual-media playback/export, never canonical approval. Source-labelled LED audio activity meter before/during capture. Record/Stop directly above video. Chirp and other nonTear takes receive technique-specific motion review without Tear correction controls. Repetition highlight/dim and time zoom preserve original geometry and don't implicitly select approval. Early manual stop is described as an incomplete plan while actual timing gates remain strict. Phone durably retains received Watch Stop requests and retries after reachability, with exact reply matching; Watch guards local/newer recordings and remembers32 terminal identities so delayed Start cannot orphan an old take. Mac shows honest manual recovery instructions when Stop wasn't confirmed.

Verification: initial focused304unique/608executions,302unique passed and2calibration-test expectations failed. Those erroneous bypass expectations were corrected without relaxing calibration requirements; correction28unique/56executions passed. Watch/bridge104unique/208executions passed. Final ordering15unique/30executions passed. Python84/84 passed after current shared changes. Universal Release build and signed iOS+Watch builds passed. Final Mac build after one calibration wording correction passed; no behavioral code changed after tests. All464 current source build-input hashes unchanged. Full scripts/build.sh all remains deferred after prior user request to hurry back to hardware, not reported as passed; historical interrupted full-gate failures remain retained.

Next single physical test: in CXL choose Capture -> Movement check(no beat), Apply Authoring Setup, Record movement check. Wait for Recording started, do one slow movement, Stop and Finalize on MAC ONLY, check Watch stops, then Play whole take. If Watch remains recording, stop it manually once and report the literal status. No beat or four repetitions needed. Existing calibration/controller/camera/audio presence and trustworthy source remain required; canonical approval remains stricter than diagnostic capture.

Remaining requested capability: exact external Serato AHHH waveform/playhead is NOT implemented; current internal control-integrated sample phase does not prove external sample/transport position. Need verified external telemetry. Phone retry only works if Stop reached phone and Watch later becomes reachable; cannot force wake, and finalized Mac diagnostics aren't rewritten as successful afterward. Original capture ZIPs retained. Full report and file list: ../evidence/record-start-recovery-20260912/RESULT.md.

--- Previous handoff retained below ---


# Current: updated CXL audio recovery app installed — 2026-09-12

Work only in /Users/karlwatson/Developer/ScratchLab-CXL-Recovery-20260912/source, branch codex/cxl-auto-p21-final-gates. The user requested faster return to hardware testing; stop broad work here and give one bounded hardware action. No commits/pushes. Primary Downloads/ScratchLab and inherited unrelated WIP preserved.

Installed /Users/karlwatson/Applications/ScratchLab CXL.app, universal arm64/x86_64, strict signature PASS, SHA256 a35f82270226cb13fc050b970048b2b4d777fb25a83b4c7359c920f771e9879c. Backup /Users/karlwatson/Applications/ScratchLab CXL.previous-20260912-190120.app. Same bundle identity, permission descriptions and entitlements. Audio preference corrected to live-discovered Rane ONE MKII UID AppleUSBAudioEngine:Rane:Rane ONE MKII:B:1,2, explicit flag retained for this requested correction. User confirmed app closed first. No recordings/approvals made. CUA reset/getApp still fails with native pipe closed; observed installed PID list [14198]. Exact installed executable is confirmed running as PID14198 after getApp; do not claim window inspection. No manual relaunch is needed if its window is visible.

Implemented: Release CXL automatic selection prefers a physical Rane while honoring deliberate selections. Audio input can change between takes after camera/audio are enabled. Selection locks through startup, count-in, recording, finalization and armed/undrained MIDI ownership. Changing an enabled input immediately invalidates preflight and reconnects through the existing capture queue; stale input completions cannot restore ready flags. Failure permits another selection and explicit Enable. Camera/MIDI selectors retain prior behavior. Files: MacCaptureEngine.swift, ScratchLabDesktopApp.swift, ReferenceAuthoringView.swift, CaptureReliabilityPhase1Tests.swift, MacCameraPreviewViewTests.swift; operator quick-start and four workflow docs updated.

Verification:43 unique targeted tests /86 executions PASS,0 failures/skips/runtime warnings, comprising38/76 camera+route and5/10 actual audio-policy tests. The initial focused command used a wrong class name for the five policy selectors; they did not run there and were separately run correctly as CaptureReliabilityPhase1CoreTests in audio-policy.xcresult. Python84/84 PASS. Universal macOS Release PASS. All590 frozen build inputs retain hashes. Full scripts/build.sh all was deliberately interrupted after user asked to hurry back to app testing: retained result2597unique,2574passed,21skipped,2failed/interrupted; one host-exited-before-finish message on testOpenAndClosedFaderKeepMotionIndependentOfAudibility, one explicit canceled testSyntheticFinalizedTimingPassesAndDriftFailsClosed. Do not report this full gate as passing or assert the host exit cause proven. iOS/Watch build legs did not run. Full gate remains deferred. See ../evidence/input-recovery-20260912 for exact logs/results/receipts.

Next single operator action: in the running installed CXL app, confirm Audio input shows Rane ONE MKII. Then enable selected camera/audio before a fresh slow pilot take. Physical audio channel-pair, actual reconnect, review/export/approval and Rane72/Twelve acceptance remain unverified. Historical Watch coverage/gap validation remains open. Existing engine stop-generation completion limitation noted by review predates this patch; no new stop/restart button was added.

---

# Current: CXL source recovered to permanent storage — 2026-09-12

Work only in `/Users/karlwatson/Developer/ScratchLab-CXL-Recovery-20260912/source` for this CXL candidate. Primary `/Users/karlwatson/Downloads/ScratchLab` is an older, separately dirty checkout and was preserved unchanged. The old `/private/tmp` candidate and receipt directories vanished; the cause is not established. They were recovered from read-only local Time Machine snapshot `com.apple.TimeMachine.2026-09-11-025633.local`. This top record supersedes old path, process and device observations below.

## Recovery evidence

- Exact source copy before these workflow-document updates: 707 regular files /206,595,364 bytes, zero differences from snapshot. All500 frozen source/test/build inputs match the final September9 manifest, SHA256 `db734d336ba1504c331b64582b614616ff47110d99b5583b33f5587b1a3cca50`.
- Git is self-contained: branch `codex/cxl-auto-p21-final-gates`, HEAD `3f43345a6c6369a4b18bbb52134a1666ac505b30`,6,378 reachable objects, git fsck --full PASS, no alternates or remotes. Original index preserved;662 logical entries, nothing staged. Before recovery-doc edits, snapshot and restored status match42 tracked modifications plus42 untracked files, and canonical full-index patch hash `0f74e68a06026b130fa6e9a98aada16c556c21dcc20b80ff6a07099f080f93a5` matches. No commit/reset/checkout/clean/stash/push was performed.
- Complete source archive and standalone Git bundle are under `../evidence` and `../git-recovery`.22,071 restored verification evidence files match snapshot hashes. Original focused, full and assertion-recovery xcresults reopen successfully. Their original failure/pass states are preserved.
- This Developer recovery folder is included by Time Machine policy. That is eligibility for future backup, not proof a new backup has completed. No source changes or rebuild were needed for recovery; exact-file hashes and historical results are the verification boundary.

## Installed app and testing boundary

Installed app remains `/Users/karlwatson/Applications/ScratchLab CXL.app`, executable SHA256 `96ab0894dc6d31aa0a7661f380f78d2a95df37c0359abb1ab1d9b2d3d55fba33`, identical to September9 receipt. Opened the same installed app; current process observed as PID8135. No installation, recording, calibration, approval or permission change was performed. Rane ONE MKII is present in the USB inventory. Computer-use UI inspection fails with `Sky Computer Use native pipe closed before response`; this is a tool connection failure, not evidence the app failed.

Next bounded operator action: in CXL Setup, check that both MIDI source and Audio input show Rane ONE MKII. Do not assume source selections from USB presence alone. Then proceed one action at a time through camera/audio, relay, calibration, one slow fresh take, Stop/transfer, review playback, approval/package export and continuation. Do not accept an old take retrospectively. No fresh capture after the September9 code fix has been established here.

## Verified software and remaining risks

Exact beat binding, real-file backing playback, automatic stop, independent Retake/New scratch and actual linked Watch/beat evidence preservation are in this recovered candidate. Last focused result212unique/424executions passed; Python84passed; all platform builds and universal Release passed. Original full result4561unique/9394runs retained1unique/2execution failures from an obsolete assertion; corrected assertion recovery1unique/2runs passed. Two established calibration-export main-thread warnings remain. These are re-opened historical results, not new tests executed today.

Watch sample-gap/full-take coverage validation remains separate unresolved work; linked/hash-valid is not continuity proof. ONE MKII hardware acceptance is pending; Seventy-Two plus Twelve production acceptance remains separate. Preserve unknown/missing fader evidence and all approval gates. The pre-existing calibration warning can imply retrospective repair, but actual frozen take evidence cannot be repaired by later calibration.

Recovered reports: `../evidence/canonical-next-take-fix/repair-report.md`, `../evidence/restored-source-proof.json`, `../evidence/restored-receipts-proof.json`, `../git-recovery/final-snapshot-restored-proof.json`. Historical tools/scripts under evidence still contain old absolute paths: retain them as evidence and do not execute them unchanged. Future builds need new isolated output/cache paths and verification bundle IDs; do not overwrite the working installed app or run test hosts in its production container.

---

# Current: CXL canonical approval / take continuation installed — 2026-09-09 02:38 NZST

Software repair complete in `/private/tmp/scratchlab-cxl-auto-20260908/p21-final-gates`, branch `codex/cxl-auto-p21-final-gates`. Primary and inherited WIP preserved. No commit/push. Evidence and exact changed-file list: `/private/tmp/scratchlab-cxl-auto-20260909/canonical-next-take-fix/repair-report.md` and `repair-changed-paths.json`. Lower records are historical and superseded where they conflict.

Installed/open: `/Users/karlwatson/Applications/ScratchLab CXL.app`, PID 72599, window 13332. Universal x86_64/arm64, bundle com.machelpnz.scratchlab.cxl-authoring, version 1.0.1/build21; executable SHA256 `96ab0894dc6d31aa0a7661f380f78d2a95df37c0359abb1ab1d9b2d3d55fba33`. Strict deep signature, ps/lsof executable and screenshot `cxl-open.png` verified. Previous app preserved as `ScratchLab CXL.previous-20260909-023615.app`. Old PID58877 exited normally after confirming its media handles were read-only; no test host was running during replacement.

Implemented: immutable verified runtime-v2 exact beat assets before intent, real-file playback and four-click boundary, durable/per-take preserved assets and portable media offset, planned automatic stop with exact-token early-stop reconciliation, independent Retake this scratch/New scratch with retained history/calibration and serial guards/navigation, exact beat assets and actual linked Watch JSON/hash in exports, late Watch refresh preservation and strict package reopen identity/hash verification. Approval evidence checks remain enforced. Historical takes are not rebound.

Verification: focused 212 unique /424 executions all passed, zero skips/failures/warnings. Python84/84 passed. Original full scripts/build.sh all exited65 solely on the obsolete assertion in testPreviewReArmsOnTheEngineWindowRelease: 4,561 unique =4,504 pass/56 skip/1 fail;9,394 executions =9,280 pass/112 skip/2 fail. The test still assumed an unobserved engine, although the view was already @ObservedObject before this slice. Corrected only that assumption, preserving direct release and tracker safeguards; recovery1unique/2executions passed with zero skips/failures/warnings. Original failure remains preserved. Two established calibration-export main-thread warnings remain. Separate isolated iOS/macOS/watchOS builds and universal Release all passed. No app-code changes followed the full run. Final500inputs match source-final-2.json, SHA256 `db734d336ba1504c331b64582b614616ff47110d99b5583b33f5587b1a3cca50`; git diff --check passed.

Current hardware boundary: verified window shows no MIDI source detected, Serato Virtual Audio selected, MacBook Pro Camera selected but camera/audio disabled, relay off. Current USB inventory has no Rane match. Next bounded action: Karl connects and powers on Rane ONE MKII. Then inspect exact MIDI/audio selections before enabling camera/audio; do not use the virtual audio fallback for the Rane acceptance take. No new recording/calibration performed. Do not signal phone console78164 or reinstall phone/Watch without a new reason.

Latest user ZIP remains the NEW Battle Loop95BPM archive, SHA256 `5c14c520d26bbc1c8c733da778b96c729e79c512623fa0330353f77b8f1d2496`,17,253,158bytes, session7f75348c-da93-4c93-b9ca-6997132c9075,12.2s audio,noBeatSpec. All7rolehashes and original sidecar passed. Earlier10.5s/1447Watchsample audit is historical. A fresh take is needed; missing binding is not proof that other canonical evidence is acceptable. No automatic approval was performed.

Remaining: ONE MKII physical count-in, four slow repetitions, auto/manual Stop, Watch transfer/sample coverage, recorded playback, canonical selection/approval/package export and continuation. Watch full-take gap/coverage validation thresholds remain separately open; linked/hash-valid is not continuity proof. Production Seventy-Two/Twelve acceptance is separate. Operator guide updated at docs/cxl_quick_start.md. Pre-existing calibration warning wording may imply retroactive repair; calibration applies to new captures and cannot repair old unknown fader evidence.

---

# ScratchLab CXL — active beat and review repair, 2026-09-09

Worktree: `/private/tmp/scratchlab-cxl-auto-20260908/p21-final-gates`, branch `codex/cxl-auto-p21-final-gates`. Primary checkout and inherited WIP are preserved. No commits/pushes. Current output/evidence root: `/private/tmp/scratchlab-cxl-auto-20260909/beat-review-fix`.

## Current gate — source frozen; corrected app installed and open

The new CXL app is installed at `/Users/karlwatson/Applications/ScratchLab CXL.app` and verified running as PID **58877**. Executable SHA256 `02acc917a03188a82b60185c311cca39c0d1c9197243a099f2f22b0180369670`; universal x86_64/arm64; bundle `com.machelpnz.scratchlab.cxl-authoring`; version 1.0.1/build 21. Strict deep signature verification passed. Prior app is preserved as `ScratchLab CXL.previous-20260909-014644.app`. Receipts: `beat-review-fix/install-receipt.json`, `candidate-receipt.json`. Window 13258 was visually confirmed with Boom Bap Trainer and Preview backing sound; screenshot `cxl-open.png`. Karl is interacting with Setup. Do not quit, activate, reinstall or replace the app while the remaining tests run. CUA transport failed; this was not an app crash.

Universal Release passed (`release/build-final-2.log`). Exact cropped-WAV and actual composed WAV/video playback/Stop tests passed in both configurations (4/4, `crop-fix.xcresult`). Full `scripts/build.sh all` (`full-build-4.log`) exited 65 after two test-host interruptions in configuration 2: `testOpenAndClosedFaderKeepMotionIndependentOfAudibility` exited code 0 before finishing, and `testRestoredSingleCandidateDoesNotInheritAnotherSelectedGap` was terminated by SIGTERM. Configuration 1 passed all 3,995 XCTest tests (56 skips) and 537 Swift Testing tests. The interruption occurred around app switching; do not present the original full run as passing or the cause as proven. Python 82/82 passed. Platform build legs did not run because the test leg failed.

The two interrupted tests are being rerun in both configurations without app switching: exec session **4591**, `interrupted-tests-rerun.log` / `.xcresult`. Then run the remaining iOS, macOS and Watch build legs via `beat-review-fix/bin/xcodebuild`, retaining separate results. Source remains frozen at `source-final-6-before.json` (458 inputs), manifest SHA256 `e3b39c5ac5f5e96ef64a2007e3cf52d9d0ccddf8764a6a921edec921e02e1592`. Do not edit runtime/tests absent a genuine failure. Final records must distinguish original full-run interruption from focused recovery and separate platform builds. All older staged candidates and stopped gates are superseded, not passes.

Implemented: selectable procedural backing sounds (default Boom Bap Trainer) and preview; four-click count-in followed by the selected drums on one audio-player schedule; mode/swing persistence through bridge/config/export; whole-take and repetition WAV/video playback independent of missing BeatSpec; one composition/player, media endpoint and cancellation guards; explicit host-proven media origin for new captures preserved through lifecycle/export; shared beat-to-media conversion for playback/fader validation/package trims; measured WAV sample rate for duration; exact partial-read-safe WAV cropping and finalized output; consolidated approval reasons and retained-take read-only controls; concise setup/review help.

Still required: finish focused recovery and platform builds, compare frozen hashes, update TASKS/DEV_LOG/handoffs with exact evidence. Next bounded operator action is **Preview backing sound**, no performance yet. Phone console **78164 must not receive signals**; no phone/Watch reinstall is needed. Historical PID 41283 references below are superseded by PID 58877.

## Existing take and remaining approval gap

Original ZIP `/Users/karlwatson/Downloads/session_2026_09_09_cxl_tear_95_bpm.zip` is unchanged, SHA256 `77c37eb062457817e07dd36088b73770eb775f93a124109e7312be47f05066cb`. Seven manifest roles/hashes match; recorded WAV10.5s, clicks-only configuration; WatchStop/transfer exported with1447 ordered finite samples spanning14.391162s, maxinterval9.954ms. This proves this artifact's contents/internal continuity, not enduring wrist-down reliability or exact cross-device alignment. Reports/media are in `take-audit-0046`. A byte-identical playable original MOV copy (audio+video) is now `/Users/karlwatson/Downloads/session_2026_09_09_cxl_tear_95_bpm_review.mov`, SHA256 `cc1eec38a34f4d9748f2eb403b798456916c8dcb1bf8107eb2bb4337c235882c`.

Karl's01:10 screenshots confirmed missing BeatSpec and retained non-reviewing take. `bindBeatSpec` has no runtime caller; selected procedural sound is not immutable beat-asset provenance. Existing six pilot assets cover80/90/100/110BPM, not95, and have a different count-in. Canonical approval therefore remains blocked. Do not attach hashes retroactively or weaken checks. A separate exact-asset capture slice must verify/select assets before capture, actually play those assets, and persist/export that same identity. Current UI now shows these blockers together; raw playback/export remains available. Older take metadata without mediaTimeOrigin retains its existing ranges; no migration or rewriting of the original archive occurred.

Known separate open work: Watch continuity/coverage validation accepts too little evidence; approved-reference package omits actual Watch artifact despite linked sidecar. Physical Rane ONE MKII and Rane72/Twelve production acceptance remain distinct and unfinished.

## Latest Watch stability check — 2026-09-09

Karl reports immediate Watch availability loss when lowering his wrist and recovery when raising it. He subsequently enabled Wake On Crown Rotation and reports improved stability. Added that operator-reported workaround to `docs/cxl_capture_run_sheets.md` and `RANE_PHYSICAL_HANDOFF.md`; it remains provisional until a complete take proves arm-lowered sample coverage, Mac Stop, transfer and Save. Do not claim a universal setup requirement, permanent connection lock, or verified background recording. Wake Duration was already 70 Seconds; Return to Clock was changed from Always to two minutes, and another setting change was not proven to fix this. Do not repeat the earlier display-setting loop.

The Mac–iPhone TCP connection remained ready while the phone forwarded paired/installed Watch availability with reachability switching true/false. The current screen evidence `.../cxl-ready-release/wrist-down-current.png` shows calibration in force, Watch unavailable and no recorded take. Therefore the idle drop does not prove sensor samples stopped during recording. Watch capture uses CMMotionManager without an extended/background runtime; immediate WatchConnectivity availability is not pairing. No runtime edit, build, reinstall or recording occurred during this investigation. A read-only audit is checking whether existing artifact validation rejects measured motion gaps; do not treat a linked filename as coverage proof.

Preserve Mac PID 41283 and phone console session 78164 (`/private/tmp/scratchlab-cxl-auto-20260909/cxl-ready-release/iphone-relay-restart-0029.log`); catchable signals forward to the phone app. Next bounded user action is Record Draft once, without scratching, to hear the four-beat count-in. Actual Stop and Watch file coverage must then be checked. Software gates remain prior evidence only; physical acceptance remains pending.


Read-only audit follow-up: current Watch association can accept an identity-matched artifact with only 10 samples, a recorded end and positive sensor duration (`CaptureReliability.swift:7,595–604`). That becomes `.linked` through `StagedCaptureRecovery.swift:1529` and `ReferenceAuthoringCaptureBridge.swift:967`; `ReferenceValidation.swift:715–717` accepts the linked filename without checking sample gaps/full-take coverage. This is a source-identified validation gap, not a physical reproduction. Manually inspect sample timestamps before accepting the pilot; a separate focused continuity-validation fix is still needed.

Separate source-audit follow-up: `ScratchLabDesktop/Services/ReferenceApprovedPackageCoordinator.swift:86` copies the reference sidecar but not its linked Watch artifact. Review approved-package Watch artifact preservation in a separate slice; raw SessionExportCoordinator export is a distinct path. No export reproduction or fix was run.


## Latest physical check — 2026-09-09 00:28 NZST

Update: the phone-only restart restored the relay. iPhone PID 3409 now forwards paired/installed/reachable Watch heartbeats; Mac connection C3 reached ready at 00:28:43. A fresh screenshot shows Apple Watch `Connected, motion idle`, and Karl confirms `works now`. No reinstall or permission change was needed. Phone console remains attached as exec session 78164 at `/private/tmp/scratchlab-cxl-auto-20260909/cxl-ready-release/iphone-relay-restart-0029.log`; do not send a catchable signal because devicectl forwards it to the phone app. Mac PID 41283 and working camera/audio were preserved. This proves current connection, not recording/Stop/export. Next bounded action is Reuse Saved Calibration: engine preflight sees a calibration but the authoring session still reports none in force and Record Draft is disabled. The proposed listener recovery bug remains unconfirmed as the cause of the pre-restart failure.

Karl confirms camera/audio work, but Watch is still disconnected. Installed Mac PID 41283 remains the verified stable CXL app. Its `_scrcamfeed._tcp` NWBrowser reached ready at 00:24:47; the available log shows no discovered endpoint, connection, or permission-denial error. Log preserved at `/private/tmp/scratchlab-cxl-auto-20260909/cxl-ready-release/relay-discovery-0028.log`. Device inspection finds the iPhone app running as PID 3169, version 1.0.1 build 21; this supersedes the old build-23 receipt but does not establish source parity. No reinstall is justified yet. Next: bring iPhone ScratchLab to its Capture/session-setup screen, leave it visible, and observe the Mac relay. Do not record. Source audit found a conditional listener-recovery gap: failed/cancelled listener creation can leave `isAdvertising` true, preventing reentry from restarting it; this is not yet established as the physical cause. No runtime source, app, or permission changed in this diagnostic.

## 2026-09-09 — Capture-ready candidate ready for one slow physical run

The direct Release route now forwards Watch Stop, capture starts only after the audible four-beat count-in boundary, and Save waits for an acknowledged Watch transfer to resolve before validating its exact evidence and source binding. Raw diagnostic capture remains possible when Watch evidence is terminally missing or failed; pending evidence cannot be silently saved.

Fresh focused gates passed 120/120 session/view-model executions, 92/92 Watch Stop/bridge executions, and two runs each of the exact relay and realtime-ingress stress tests. Fresh `scripts/build.sh all` passed Python 82/82, XCTest 7,960 executions with 110 skips and zero failures, Swift Testing 537 tests, and iOS/macOS/Watch builds. The universal Release candidate at `/private/tmp/scratchlab-cxl-auto-20260909/cxl-ready-release/products/Release/ScratchLab.app` is `x86_64 arm64`, bundle `com.machelpnz.scratchlab.cxl-authoring`, and strict deep-signature verified with the established sandbox entitlements.

Next: install that candidate at `/Users/karlwatson/Applications/ScratchLab CXL.app`, preserve the previous app as a timestamped backup, open the exact installed path, verify its PID/executable/hash, then give Karl one instruction at a time. First confirm the four count-in beats without performing. Only then record four slow repetitions once, press Stop on the Mac, verify Watch stops, wait for linked Watch evidence, and Save Capture. Stop at the first physical failure. Do not mark the TASKS item complete from software evidence alone. The Rane Seventy-Two plus Rane Twelve production rig remains a later separate validation.

Installation completed: the prior stable app is preserved at `/Users/karlwatson/Applications/ScratchLab CXL.previous-20260909-001920.app`. PID `41283` runs `/Users/karlwatson/Applications/ScratchLab CXL.app/Contents/MacOS/ScratchLab`; `ps` and `lsof` agree, and installed SHA-256 `cd1e106b10366d00e08bd6e674515c22a102190457fed716d60116b093b63e82` matches the candidate. A local screen capture confirms Setup visible with Rane ONE MKII MIDI/audio and MacBook Pro Camera selected, while camera/audio and relay are off. Next physical action is **Enable Selected Camera & Audio** once.

Candidate `/private/tmp/scratchlab-ttm-prompts-9-12-20260907T133704Z/candidate`, branch `codex/ttm-comparison-export`, unchanged base `3f43345a6c6369a4b18bbb52134a1666ac505b30`. Prompts 9–12 are complete. No commit, push, hardware capture, installation, publication or training action occurred.

Prompt10 adds the optional `scratchlab_reference_tear_evidence_v1` companion without changing session-export v4, capture_spec v1, detected-notation v1, replay v1 or approved Reference Package v1. It binds exact finalized raw sidecar bytes/hash and captured session/take/reference identity to the full Tear review, stored canonical projection and intrinsic limitations. Missing companion remains legacy/not analysed; malformed, future-version, identity/hash mismatch and late source rewrites fail explicitly. Restore targets only the corresponding existing review take through its serial owner. The companion does not contain the full capture recipe, approval decision or whole-session restore contract and does not install, publish or train anything.

Prompt11 uses the final source from the repaired gate. `p10-prepare5` passed. The entire `MIDILearnEngineTests` suite passed 32 unique/64 executions. `p10-focused4` passed 753 unique cases/1,774 executions: 1,772 passed, two skipped, zero failed and zero runtime warnings across all 60 selected suites and both configurations. `p10-full2` passed in 1,205.669 seconds: Python 82/82; desktop 4,477 unique cases (4,421 passed, 56 skipped, zero failed), 9,226 executions (9,114 passed, 112 skipped, zero failed), all 267 suites and both configurations; iOS, macOS and Watch Debug builds passed. Two runtime warnings remain attached to the known calibration-export method. All 70 protected roots/91,737 entries and all 662 frozen inputs stayed unchanged during the gates.

Prompt12 semantics: N internal stationary holds divide a complete same-direction Tear into N+1 moving subdivisions. Direction, holds, fader state and actual audio are independent evidence streams. Fader click/cut events are not transition-edge or Tear counts. Motion/fader correlation predicts gated sounding regions but cannot establish audio-confirmed slices. Inferred automatic holds have descriptive counts only; their timing and ratios remain unavailable. Unknown, interrupted, ambiguous, corrected timing, missing fader and interpolated curves remain explicit. The seven comparison dimensions stay separate, with no aggregate grade or absolute pitch/acceleration claim.

Software defaults awaiting physical CXL validation: decoder CC6/channel1, modulus128, moving run 0.08s/eight steps, packet gap0.10s; review ambiguity0.12s, confidence0.75, time tolerance0.000001s, reversal confirmation0.25s, chatter delta0.015 normalized; fader closed/open0.12/0.62 and dwell0.008s. Comparator timing50ms, duration-share0.10, normalized shape RMS0.10, confidence0.75 and continuity0.000001. Pure segmenter defaults remain separately tested and are not runtime-capture or physical-calibration proof.

The production canonical beat-target registry remains Baby-only. Internal authoring vocabularies are broader, but no existing recording/reference became valid, approved, published, restored from withdrawal or training-eligible. Later order remains a proposal: plain Tears → clicked/flare Tears → Tear-containing orbits → compound combinations.

Next authorized work is the CXL operator-release phase in a fresh isolated candidate. Apply the reviewed roadmap/run sheets/evidence board, then implement the first bounded `Next take` progression slice. The focused Figma inventory/organisation and CXL operator flow design occurs before the later operator-layout implementation, using the actual existing Figma file. Preserve public-beta learner designs and keep CXL navigation Session Setup → Capture → Review & Export. Hardware, recording, upload, publication and commit/push boundaries remain separate.

---

# Historical authorized MIDI fixture repair

Candidate `/private/tmp/scratchlab-ttm-prompts-9-12-20260907T133704Z/candidate`, branch `codex/ttm-comparison-export`, base `3f43345a6c6369a4b18bbb52134a1666ac505b30`. Karl authorized the separately reviewed one-method MIDI fixture repair by saying “proceed.” Prompt9 is complete; Prompt10 is implemented but awaits fresh gates; Prompt11/12 remain incomplete.

Fresh reviewed source repair passed p10-prepare4. p10-focused3:60exact suites,753unique (751pass/1skip/1fail),1774executions (1771pass/2skip/1fail),zero runtime warnings; both configurations fully audited. The only failure is unchanged MIDILearnEngineTests.testLearnedHotCueSuppressesTempAhhhFallbackOnSameEvent at238 inConfiguration2; the firstconfiguration passed. Earlier full failed the opposite configuration; isolated diagnostic2/2passed. Cause remains unproven. The17method Tear pipeline,25codec and12writer suites passed in both configurations; both repaired finalized-projection assertions passed. p10-full2 NOT RUN.

See ../P10-SECOND-FOCUSED-FAILURE.md, ../P10-MIDI-REPEAT-REPAIR-PROPOSAL.md and ../P10-MIDI-PROPOSAL-REVIEW.md. The approved repair changes only `MIDILearnEngineTests.testLearnedHotCueSuppressesTempAhhhFallbackOnSameEvent`: fixed sleeps become existing persistence-queue plus main-publication barriers, with explicit learned-control and assigned-sample preconditions. The original MIDI events, playback barrier and final fallback assertion remain. Production and adjacent MIDI tests remain unchanged. Freeze the exact source, then run the full MIDI suite, the 60-suite plan and `scripts/build.sh all` sequentially; stop and preserve any failure.

Current tested digest8874843f840b567eba0520a61baaffaeddc0a386f8c09dd3d28ab448a53544c2; frozen patchab41a201192a21df9b10a07fd68eadfd1cd2d4c2fd019844e3f3c7186c6567ea. The post-test protected-directory read was interrupted; one fresh read-only recheck passed91737entries/70roots withzerochanges. All four prior failure baselines match (2955,20,4449,19951entries). All13originalworktrees andordinaryrefs unchanged; one Codex bookkeeping ref difference is recorded separately. Only these four workflow records were finalized after the gate; see blocked-stop source proof. No failure is reclassified as a pass.

The requested pre-Prompt11 CXL review is COMPLETE. ../CXL-PLAN-REVIEW.md records next-take,media,timing,identity,late-Watch,approved-package andRelease gaps. Formal CXL roadmap/run-sheets/evidence-board application and the first explicit Next take implementation remain authorized AFTER successful12 and completion report. External drafts retain32queue rows/minimum40variants/160accepted-take slots; all acceptance/content/rights states are pending. Preserve every ../QUEUED-CXL*.md direction, including the latest Figma design/handoff slice before operator-layout implementation. Discover the correct existing Figma file only when that slice starts, preserve learner designs and map proposed states to actual shared contracts. No Figma edits have happened.

CXL presentation remains direct Setup→Capture→Review & Export, excluding consumer Practice/coaching/progression/lesson/experimental launch/navigation. Companion changes only for supportedcapture roles. Clean references and separate narration assets, complete public-beta curriculum/content, original notation, rights/captions/backups andactualRelease/physical acceptance remain required. Model policy is ../MODEL-SELECTION-POLICY.md:Sol/Mediumdefault;Astra/Highselectively;no self-switch was claimed. No commit,push,hardware,capture,installation,publication,training,upload orcleanup authorization is added.

---

# Historical completed slices — retained context
...
Root cause: Reference Authoring originally created its tracker only during recording, while its MIDI accumulator needed an open epoch. Preflight read the separate lifetime CC cache, so healthy preflight did not prove chart input. The first correction then retained a cached-epoch bypass, inadequately scoped take release, unproven publisher observation, and unsafe out-of-order retention bounds. This continuation completes the interrupted second correction; hardware acceptance remains pending.

STATE AND SCOPE. Worktree `/Users/karlwatson/Downloads/ScratchLab-TTM-PreRecordNotation`, branch `fix/reference-authoring-prerecord-live-notation`, unchanged base/HEAD `7ba70916546829a5b2fceb8559cc0934c0a798c4`. Empty index; zero untracked files. The initial audit found NINE modified files, not the requested eight: `CaptureReliabilityPhase1Tests.swift` was already dirty. Preserving that existing ninth file and its compatibility changes takes precedence over forcing an eight-file claim. No current hunk was reset, reverted or discarded. Incoming binary patch `/private/tmp/scratchlab-prerecord-second-finish-20260907T051832/incoming.patch`, SHA-256 `dec753739be98fdcc60ecc5f2065955e1c0906a8a534b81209b09a0647cf228c`. Final exact diffstat and binary patch SHA-256 are in `final-diffstat.txt` and `final.patch.sha256` in that evidence directory; the patch includes all nine files and all four corrected workflow documents.
...
- `ScratchLabDesktopTests/LivePerformedNotationTrackerTests.swift`
- `ScratchLabDesktopTests/CaptureReliabilityPhase1Tests.swift`
- `ScratchLabDesktopTests/MacCameraPreviewViewTests.swift`
- `AI_HANDOFF.md`
- `AI_HANDOFF/next_prompt.md`
- `DEV_LOG.md`
- `TASKS.md`

ALREADY PRESENT. The interrupted candidate already contained the preview/take presentation modes, a single tracker construction site, lock-owned idle/preview/take state, ingress-ticket propagation replacing the bare cached epoch, preliminary take tokens, explicit onReceive subscription, min/max timestamp retention accounting, and 35 genuinely new tests. It had already removed all five caller-epoch arguments across four existing tests. The workflow documents still described the older first correction and its smaller test/file counts. Prior evidence directories `scratchlab-prerecord-notation-20260907`, `...20260907b`, and `...20260907c` remain historical and are not evidence for this final source.

CODEX CHANGES IN THIS CONTINUATION. Append compares the complete ticket, including owner, generation, epoch and optional token. The single generation also supplies each armed token's unique value, removing the extra token sequence. The token carries its immutable media URL so a delegate may acquire only the token for that recording; that same token crosses the asynchronous audio/mux and finalization stages. Both finalization stages refuse stale tokens, and a mismatched drain returns without continuing finalization. Arming refuses an undrained take; the real start entry point refuses it before changing camera/duration/sidecar/audio state, and the catch abandons only the optional token actually obtained by that start. Epoch open/close also require a matching token. Existing preview/time retention behavior is retained. The view keeps its plain-let engine, @State presentation fields, and one tracker constructor; its onReceive handler and lifecycle transition now share directly executable functions with the tests. Replayed, duplicate and stale release counts are ignored, and off/take presentation modes cannot re-arm a preview. Three existing capture regressions now explicitly arm and open real take windows.

FINAL INVARIANT. One `midiCaptureLock` owns idle/preview/take, one monotonically increasing generation, the epoch, one optional unique take token, and the existing event array. A take token exists exactly under take ownership. Real `receiveMIDIPacketList` captures one whole ticket under that lock and carries it through parsing/dispatch; immediately before append the same lock validates ALL ticket fields, rejects idle and epoch zero, calculates takeRelativeTime solely from the ticket epoch, and appends atomically. No caller epoch parameter exists. Arming clears preview events before establishing a take epoch. Stop closes admission but retains take ownership/evidence. Only a matching drain or abandonment transitions take to idle and publishes one release on DispatchQueue.main, after unlocking. Stale/duplicate operations publish nothing. isRoutineRecording/isRoutineFinalizationPending are not MIDI ownership authorities; the existing finalization-pending gate still separately protects the wider recording pipeline until sidecar finalization finishes.

RETENTION. Preview only: provisional 15-second trailing retention, existing amortized trim trigger at a 30-second held span, strict maximum 32,000 held events, and a 24,000-event count-trim target. Min/max timestamps handle out-of-order arrivals; trimming filters by timestamp rather than dropping an assumed-old prefix. The 15-second policy is not a claim of an exact 15-second post-append span: the retained amortization allows up to 30 seconds between trims. Idle expiry occurs only on a later append that triggers trimming; there is no timer expiry. The real append tests reach 32,000, 32,001 and 33,000 preview arrivals (final held count 24,999), and a take retains/drains all 33,000 events exactly once. No persisted/export schema, decoder, renderer, second buffer, or capture-ownership UI flag was added.

EXECUTABLE PROOF (all pass in both configurations; the LivePerformedNotationTracker cases also pass three iterations). The DEBUG pause hook is inside the real ticket-to-append interval, outside the lock, with bounded semaphores/expectations and no sleeps. Source assertions supplement these tests and are not the concurrency proof.

1. `testRealMIDIIngressPreviewTicketRetiresAcrossArmingAndMediaStart` uses a real CoreMIDI MIDIPacketList and pauses after its preview ticket, across arming with epoch zero and across media start. Both packets are explicitly rejected. `testPreviewPacketPausedAcrossRecordArmingNeverEntersTheTake` also pins the carried preview owner/token.
2. `testTakeWindowWithAZeroEpochRejectsEveryPacket` exercises arming and stopped epochs; no packet is admitted.
3. `testSameGenerationCannotOverrideTicketOwnerEpochOrTakeToken` supplies malformed tickets with the CURRENT generation and altered epoch, owner or token; all reject. A matching ticket calculates relative time from its own epoch. `testNoCachedEpochCanOverrideTheActiveTicket` also pins removal of the old production parameter.
4. `testAStaleTakeTokenCannotDrainOrAbandonAnotherTake` leaves the new take's four events intact; `testDelayedMediaCallbackCannotAcquireAnotherTakesToken` proves old media URLs and cached delegate tokens cannot obtain/drain/release the successor.
5. `testAFailedStartCannotAbandonAnotherTakesStoppedEvidence` calls the REAL start entry point against a stopped take, verifies explicit failure, also exercises refused arming, and preserves/drains its six events.
6. `testDuplicateDrainAndAbandonmentPublishNoRelease` proves only the first matching drain publishes. `testNoSidecarFinalizationReleasesOnlyItsCapturedTokenOnce` calls actual no-sidecar finalization twice and observes one release.
7. `testOneRealReleaseCausesExactlyOnePreviewReArm` receives the real publisher on main, invokes the SAME handler used by onReceive, proves one reopened preview, and rejects replay/duplicate/stale counts without another generation. This does not claim a mounted SwiftUI/hardware acceptance test.
8. `testOutOfOrderPreviewTimestampsRemainCorrectlyBounded` exercises shuffled timestamps through the real append path and checks min/max bounds and retained recent events.
9. `testPreviewAppendCapsAtTheRealConfiguredEventCeiling` checks the exact 32,000 and 32,001 boundaries plus 33,000 total arrivals through production append.
10. `testATakeLargerThanTheCeilingIsNeverTrimmedAndDrainsOnce` retains/drains 33,000 events and refuses the second drain.
11. `testRecordArmingClearsPreviewDataBeforeTheTakeEpochExists` proves empty evidence while the armed epoch is zero. Paused take packets across Stop, drain and subsequent preview are rejected by `testTakePacketPausedAcrossStopIsDroppedNotAppendedAfterMediaEnd`, `testTakePacketPausedAcrossTheFinalDrainCannotRaceIt`, and `testStaleAppendAfterTheDrainCannotContaminateTheNextPreview`. Preview begin/end during arming and stop/finalization gaps cannot mutate the take.
12. `testRepeatedRouteEntryCreatesNoDuplicateTrackerOrWindow` calls the production view lifecycle transition and verifies tracker object identity/window generation across duplicate entry, exit, restored/re-entry, and take transitions. `testRestoredRouteEntryResolvesTheModeExplicitly` additionally pins the actual task wiring. No second tracker construction site is present.

TEST ACCOUNTING. Against HEAD: 39 genuinely new tests, seven updated existing tests including one rename, zero removed tests. This continuation: four new tests and 12 updated tests relative to incoming.patch (nine LivePerformedNotationTracker tests and three CaptureReliabilityPhase1Core tests); MacCameraPreviewViewTests was already modified and is unchanged by this continuation. The rename from testLeavingTheRecordingPhaseClearsTheAuthoringTracker to testLeavingTheRecordingPhaseRebuildsTheAuthoringTracker counts once as an UPDATE. Exact names/comparisons are in test-counts.json, generated from method bodies rather than nearby added comments.

FINAL VERIFICATION. `prepare3.xcresult` / prepare3.log: isolated macOS Debug build-for-testing PASS. `focused3.xcresult`: 373 unique selected tests, 372 passed / one skipped / zero failed in EACH of Test Scheme Action and Configuration 2; 746 expanded runs = 744 passed / two skipped / zero failed. Selection: LivePerformedNotationTrackerTests, MacCameraPreviewViewTests, CaptureRecoveryPhase2CoreTests, MIDILearnEngineTests, MIDILearnHangFixTests, ReferenceAuthoringRouteActivationTests, ReferenceAuthoringTests, ReferenceAuthoringViewModelTests, ReferenceAuthoringCaptureBridgeTests, ReferenceAuthoringSessionTests, plus the three updated CaptureReliabilityPhase1Core tests. `repeat3.xcresult`: 76 unique tests; 225 passed / one skipped / zero failed in EACH configuration, 452 expanded runs = 450 passed / two skipped / zero failed. The 75 non-skipped cases run three iterations; Xcode does not repeat the skipped case. Both final summaries report zero runtime warnings. The sole skipped test is the pre-existing testTake003RealStreamIsNotFlattenedByTheLivePath (artifact absent in the isolated test environment). `compile-final2.xcresult` / compile-final2.log: separate isolated macOS Debug compile PASS, sequential after tests. No warning in either changed production file; existing unrelated FileManager/Sendable and test warnings remain. git diff --check passes. No Release, iOS, Watch, production bundle ID or scripts/build.sh/full gate was used.

ISOLATION. Host `com.machelpnz.scratchlab.prerecordfinish.20260907e.ScratchLabDesktop`, tests `com.machelpnz.scratchlab.prerecordfinish.20260907e.ScratchLabDesktopTests`, containers absent before use. Built identifiers, both established configurations and signed host App Sandbox verified in identity-verified.json. Each invocation checks no competing xcodebuild and records exact argv/environment. Explicit derived data, SYMROOT, OBJROOT, precompiled-header/module/cache roots and result paths are under `/private/tmp/scratchlab-prerecord-second-finish-20260907T051832`; the separate compile uses CompileDerivedData/CompileProducts/CompileIntermediates. Tests launch only their isolated harness; no interactive app or stable host was launched/relaunched/terminated.

RETAINED ATTEMPTS, NOT FINAL PASS EVIDENCE. prepare1 and prepare2 both passed on earlier versions; focused1 passed with 372/1/0 per configuration before the final stale-finalization guard. focused2 FAILED with 371 passed / one skipped / one failed per configuration (742/2/2 combined): a source-string assertion still expected the removed flatMap drain. It was corrected to REQUIRE the new matching-token guard, not removed or relaxed. focused2.xcresult, log, source hashes and command/result JSON remain separately named and intact. compile-final exited 64 before compilation because -testPlan was passed to build; compile-final2 removes that test-only argument. Sandbox process-query/report-cache failures are separately recorded and were rerun with escalation. No failed artifact was overwritten. Earlier ...20260907c build/focused failures and all earlier evidence directories remain retained and superseded.

HISTORICAL COUNT CORRECTION. The ...20260907b repeat3 xcresult actually reports 186 passed / ONE skipped / zero failed per configuration = 372 passed / TWO skipped, 374 expanded runs. The old claim of three skips per configuration / 378 combined runs was incorrect. The interrupted ...20260907c repetition records 213 passed / one skipped per configuration = 426 passed / two skipped, 428 runs. These were read from existing xcresults, not rerun. The earlier rejected candidate's 212.421-second observation for the pre-existing, unmodified, passing testAuthoringLiveAndReviewShareOneTearProjection remains UNEXPLAINED; no cold-cache explanation or hardware inference is asserted.

PRESERVATION. before.json/after.json and after-comparison.json record the exact production baseline: 83,567 regular files + 34 symlinks = 83,601 entries. The final snapshot is 83,572 regular files + 34 symlinks = 83,606 entries: FIVE added and ELEVEN changed .DS_Store files, zero removed files. Every difference is .DS_Store; its writer is not established. These files were neither restored nor deleted. All non-.DS_Store production entries, including every recording/session artifact, are byte/metadata-identical. A blanket claim that the entire production container is unchanged would be false. preservation-exception.json lists the exact paths. No added/removed/changed entry was found in hardware container (37), prior hardware artifacts (6,174), protected preview (77), shared DerivedData products (134), both older camera-restore test-container baselines (36 entries and one absent marker), and the complete installed stable app (25 entries). All nine extant OTHER worktrees retain identical tracked/untracked file hashes and metadata, HEAD, status, staged and unstaged binary diffs; the three historical missing worktrees remain missing and the worktree inventory is unchanged. Session 41949897-5458-449d-9280-65508a4f6600 remains 65 byte-identical files (53 RoutineCaptures + 12 AuditSummaries); Takes 001–008 present, Take 009 absent. Stable executable SHA-256 remains 908bbff4f994c81ef38a0b48c24ea81f4de65f56e0a4584e8fab425e81bc8975; PID 79259 and its launch time are unchanged and it was never signalled. The eleven protected configuration files match HEAD; no persisted/export schema change. No stage, commit, push, recording, recalibration, approval/rejection action, export, publication, installation of reference data or training occurred.

REMAINING RISK / NEXT ACTION. Hardware acceptance is PENDING. The installed stable host `/Users/karlwatson/Applications/ScratchLab-HardwareTest.app` still contains the OLD COMMITTED BUILD, not this correction. Software barriers and publisher-handler tests do not establish physical MIDI delivery, AVFoundation error/cancellation behavior, actual mounted SwiftUI navigation, or visible pre-record motion on the Rane rig. The retention numbers remain provisional. The unexplained production .DS_Store metadata drift remains an explicit preservation exception for independent review. Next action is the independent read-only checkpoint review in AI_HANDOFF/next_prompt.md and `/private/tmp/scratchlab-prerecord-second-finish-20260907T051832/CHECKPOINT-REVIEW-PROMPT.md`; no checkpoint or hardware action is authorized by this work.

---

# 2026-09-07 — Isolated Reference Authoring restored-route startup (software verified; uncommitted)

Selected Karl's explicit one-slice request over the unrelated first unchecked backlog task. Repair worktree /Users/karlwatson/Downloads/ScratchLab-TTM-CameraRestore, branch fix/reference-authoring-restored-camera-start, exact base/HEAD ae4d1ca9ab81f1e43f92259e1b40f65fd6e38e8d. Canonical clean/index-empty/untracked-zero baseline and Prompt A/B/C/camera ancestry verified before creation; new repair worktree clean at that exact base before editing. PID 35367 verified on the requested hardware path and left running.

The restored authoring view mounted a passive preview and started preflight polling, but never requested live capture startup; MacAnalyzerView's parent startup was gated by the saved global liveInputEnabled preference. ReferenceAuthoringView's existing task now calls activateCaptureInput(), which requests MacCaptureEngine.start() on MainActor. The existing engine-owned synchronous isRunning guard rejects repeated/reentrant requests before permission/configuration work; view/window recreation shares that guard, while each fresh engine gets its own request. Leaving the route does not stop the shared engine. The preference, selected-camera policy, permissions/discovery/session queue and PreviewAttachment are unchanged. A nil-by-default DEBUG override substitutes external startup work below the real guard for deterministic tests; normal startup is unchanged.

Files: ScratchLabDesktop/Views/ReferenceAuthoringView.swift (+10), ScratchLabDesktop/Services/MacCaptureEngine.swift (+12), ScratchLabDesktopTests/MacCameraPreviewViewTests.swift (+131/-4), plus TASKS.md, DEV_LOG.md, AI_HANDOFF.md and AI_HANDOFF/next_prompt.md. No new repository file or project/scheme/membership edit. Implementation/test boundary: +153/-4; exact final workflow-inclusive diffstat is in final-review-receipt.json.

Software verification: 25 existing MacCameraPreviewViewTests plus eight ReferenceAuthoringRouteActivationTests, 33 discovered/executed/passed in EACH of Test Scheme Action and Configuration 2; 66 total executions, zero failures/skips/runtime warnings. Isolated macOS Debug build-for-testing and subsequent macOS Debug compile PASS. The initial test build failed because the new test referenced a file-private defaults key; fixed in the test by using its existing string value before any test ran. Original initial-prepare artifacts are retained. Sandboxed codesign verification failed; the authorized verification outside the sandbox passed before testing. Inherited Sendable/FileManager and existing test unused-value warnings remain; no new-code warning. No iOS, Watch, Release or scripts/build.sh/full gate was run, per Karl's narrower explicit instruction.

Isolation: /private/tmp/scratchlab-camera-restore-20260907a/ holds exact argv/environment JSONs, logs, xcresults and SHA-256 receipts. Host com.machelpnz.scratchlab.camerarestore.20260907a.ScratchLabDesktop and test bundle com.machelpnz.scratchlab.camerarestore.20260907a.ScratchLabDesktopTests were verified from built plists/signatures; tests inherit the signed host's App Sandbox. Both containers were absent before testing. Both established configurations resolve to that host. Each invocation checks for no competing xcodebuild and uses explicit derivedDataPath, SYMROOT, OBJROOT, SHARED_PRECOMPS_DIR, MODULE_CACHE_DIR and result paths, plus isolated ancillary caches. Shared DerivedData products were not overwritten.

Preservation: SHA-256, file metadata and inventory comparisons show zero new/changed/removed files in production (83601 entries), the running hardware container (37), /private/tmp/scratchlab-hardware-20260907d (6174), the protected earlier preview container (77) and shared products (134); counts include symlinks. All eight pre-existing extant worktrees remain byte/status/diff/index-identical, including canonical Prompt C. Three already-missing historical worktrees remain recorded as missing; none was repaired. The requested new worktree registration/branch is the sole intentional shared Git inventory addition. Eleven protected configuration files are unchanged. No interactive app launch, PID termination/relaunch, camera change, recording, calibration, export, approval, installation, publication, training, staging, commit, merge or push.

Software verification does not establish hardware camera-restoration acceptance. The seam deliberately excludes real permissions, camera discovery and AVFoundation startup. Phase 1 remains pending: five entry/exit cycles, navigation during configuration/start/stop, a safe device change, resize, window recreation and saved-camera restoration with liveInputEnabled false after a separately authorized normal quit/relaunch. No new live-notation work was implemented. Recording requires successful preview acceptance and separate explicit authorization.

Current next action: read-only candidate review in AI_HANDOFF/next_prompt.md. Earlier entries below are historical; Prompt C is now committed at ae4d1ca, and this seven-file camera-start candidate is NOT committed.

---

# 2026-09-06 — Prompt C coordinate contract and Review truth (uncommitted, software verified)

Worktree `/Users/karlwatson/Downloads/ScratchLab-TTM-Reconstruct`, branch `feature/ttm-crossfader-takestart-reconstruct`, base `9bb82d1` (Prompt B, now COMMITTED — the entry below still describes it as uncommitted and is historical). Prompts A (`cb4adc5`) and the camera repair (`63e2898`) are ancestors and present. Began clean with an empty index, so the entire candidate belongs to this slice and no pre-existing dirty file was touched.

Audited mismatch, resolved. `CaptureCore.decodePlatterCore` span-normalises each take's integrated platter position to 0…1 over that take's own decoded range and writes those fractions into every `source: "controller"` movement event finalization persists. `ReferenceTearCanonicalProjectionBuilder` stamped every record it produced with a hardcoded `static let coordinateSpace = .platterRevolutions`, so normalized take-local values were handed to a record whose declared space said revolutions. The LIVE path reaches the same projection through `derivePlatterMovementEventsWithProvisional`, which divides raw CC6 steps by 3,600 and genuinely is revolutions — two divisors, one label.

Coordinate decision. A third `ScratchNotation.GestureRecord.CoordinateSpace` case, `normalizedTakeLocalDisplacement`, plus a named contract `CaptureCore.PlatterNotationCoordinates` (basis + non-empty `reference`). Defaults are always the NON-CLAIMING basis, `calibratedRevolutions` returns `nil` for an unusable divisor or missing reference, and `raneOneMKIIDirectMIDI` fails closed to take-local with the reason stated. The review stores the basis; the projection reads it. FINALIZED reference takes declare NORMALIZED, because `ReferenceTakeEvidence` carries no raw mixer MIDI and no steps-per-revolution reference — a revolution claim there would have to be fabricated, and re-deriving from raw MIDI would change the magnitudes the unchanged review thresholds are written in. LIVE declares CALIBRATED via `LivePerformedNotationTracker.platterCoordinates`, which knows which branch of `computeState` ran and falls back to non-claiming for camera evidence. `source` alone can never settle the unit.

Parity is established on time, grid, direction and hold structure, with positions equal after each is rescaled onto its own span — exactly what `CanonicalFrame` does at draw time, and both bases are linear in raw steps with a constant positive divisor. Space/frame mismatch and contrary-direction curves still render explicit MOTION UNKNOWN and manufacture no horizontal Tear hold. No detection threshold, noise gate or segmentation parameter changed, no approval or training gate weakened, raw evidence retained verbatim.

Review truth. `advisoryDetectionStatement(for:)` labels the Baby-only audio detector LIMITED — not a disagreement — on a Tear-selected take (`MacScratchDetector` runs only `matchBabyScratch`; `ScratchClassifying` registers only `BabyScratchClassifier`), while a mismatch inside its real vocabulary still reads as one; it never writes `evidence.metadata.technique`. Overload is reduced by grouping and lazy disclosure only: `tearCandidateGroups` is a strict partition of `review.candidates` carrying IDs, deleting nothing and asserting nothing, with only decision-worthy buckets open by default. `tearCoordinateContractText` states the unit on the review.

Code boundary — nine files, **+1,075 / −23**: six production (`ScratchLab/Models/CaptureCore.swift`, `ScratchLab/Models/Reference/ReferenceTake.swift`, `ScratchLabDesktop/Services/LivePerformedNotationTracker.swift`, `ScratchLabDesktop/Services/ReferenceAuthoringCaptureBridge.swift`, `ScratchLabDesktop/ViewModels/ReferenceAuthoringViewModel.swift`, `ScratchLabDesktop/Views/ReferenceAuthoringView.swift`) and three existing test files (`LivePerformedNotationTrackerTests.swift`, `ReferenceAuthoringTests.swift`, `ReferenceAuthoringViewModelTests.swift`). No new repository file, no project/scheme/membership change, no export-schema change — the new types are `Equatable, Sendable` and not `Codable`, and the added enum case is additive to a `String` raw-value enum no persisted payload writes.

Verification, Debug only: macOS build PASS; macOS `build-for-testing` PASS; unsigned generic iOS build PASS. Focused run 1 = 164 tests in EACH of two configurations (328 executions); focused run 2 = 151 XCTest in EACH configuration plus the 8-test Swift Testing suite `AuthoredTearTemplateTests` in each (318 executions). Zero failures across both runs. No full `scripts/build.sh` gate and no Watch/Release gate, as instructed; Release tests remain impossible in this project.

HARDWARE VALIDATION WAS NOT RUN and is deferred to Prompt D. No app launched, no take recorded, no calibration performed, no controller unplugged, no approval, export, installation, publication or training. Takes 001–008 capture artifacts are unchanged: all 53 protected-session RoutineCaptures files retain their original evidence. The isolated preview-smoke container is unchanged. During Prompt C focused run 2, the app-hosted XCTest process used the production bundle identity and refreshed 27 derived `AuditSummaries/relayedWatch` JSON files, changing `updatedAt` metadata; those 27 files are derived audit summaries, not capture evidence, and they are retained exactly as that run left them rather than restored or rewritten. No media, sidecar, raw platter timeline or Watch motion file changed. No Take 009 exists in protected session `41949897-5458-449d-9280-65508a4f6600`; unrelated historical Take 009 artifacts exist in other sessions. `~/Downloads/ScratchLab-TTM` was neither read nor written. Nothing is committed or pushed. The shared product at `~/Library/Developer/Xcode/DerivedData/Build/Products/Debug/ScratchLab.app` is still NOT an authorized isolated hardware candidate: the path is mutable and worktree-agnostic. It historically carried `com.machelpnz.scratchlab`, which resolves to the production container holding Takes 001–008; the first Prompt C isolation attempt used `-derivedDataPath` without explicit `SYMROOT`/`OBJROOT`, so Xcode still wrote products to this shared path and overwrote the app with `com.machelpnz.scratchlab.promptcverify.20260906`, its currently verified bundle id. Prompt D still needs a fresh build with a unique bundle id and its own container.

Remaining gaps: a finalized take still cannot claim calibrated revolutions without carrying a steps-per-revolution reference beside its evidence; the 3,600 constant is a stated device measurement with no independent hardware calibration; gap/clock tolerance, decoder sampling floors, the chatter bound and the hold-ambiguity policy still need hardware calibration.

Prior entries below are historical and are not rewritten.

# 2026-09-06 — Prompt B parked-crossfader take-start repair (uncommitted, software verified)

Worktree `/Users/karlwatson/Downloads/ScratchLab-TTM-Reconstruct`, branch `feature/ttm-crossfader-takestart-reconstruct`, HEAD/base `cb4adc5512d1af5fa3f2a95824f10e187f0e6a1e` (Prompt A, `fix(capture): preserve Rane platter timing and provenance`), whose parent is camera repair `63e28981c6353ff51d859ef1490e852a94e96343`. Both are therefore present in this base. The candidate was reconstructed from the preserved Prompt B patch because the original work had been authored on `feature/ttm-tear-notation-alignment` at `863e420`, which predates both commits; `~/Downloads/ScratchLab-TTM` is left byte-identical and untouched.

Root cause: `midiConnectionGenerationStorage` advanced on every `MIDIPortConnectSource`. Recording start (`openMIDIInputForRecording`) and finalization both reconnect deliberately, so the generation moved twice per take against a fader nobody touched. That invalidated an otherwise fresh pre-take observation at two independent sites — the media-start classifier `MacCaptureEngine.crossfaderTakeStartState`, and `ReferenceCrossfaderTakeStart.correlate`, whose expected value the bridge reads live at stop time. The repair changes only WHEN the generation advances: a new `MIDIConnectionEndpointIdentity` (selected source id plus Core MIDI endpoint ref) and a pure `nextMIDIConnectionGeneration` rule hold the generation across a same-endpoint internal reconnect and advance it for every other case — different source, replaced endpoint after unplug/replug, a selection resolving to no endpoint, and the first connect. Absent identity on either side counts as a change. No identity check was weakened: the classifier and `correlate` are byte-identical to before. A second change clears the spent calibration sweep on commit, so the panel stops asking the operator to commit a calibration already saved and later fader motion can no longer overwrite it; recalibration stays explicit.

Code boundary — exactly five production/test files, **+326 / −9**:
`ScratchLab/Models/Reference/ReferenceAuthoringSession.swift` (+9/−0); `ScratchLabDesktop/Services/MacCaptureEngine.swift` (+90/−5); `ScratchLabDesktop/Views/ReferenceAuthoringView.swift` (+8/−1); `ScratchLabDesktopTests/ReferenceAuthoringCaptureBridgeTests.swift` (+100/−0); `ScratchLabDesktopTests/ReferenceAuthoringSessionTests.swift` (+122/−0). No new repository file, no project/scheme/membership change, no export-schema change. `project.pbxproj` and `CrossfaderCalibrationStore.swift` were deliberately NOT carried and remain dirty only in the TTM worktree.

Verification: macOS `build-for-testing` PASS; focused suites **75 tests in EACH of two configurations — 150 executions, zero failures** (`CrossfaderTakeStartStateTests` 13, `ReferenceAuthoringSessionTests` 39, `ReferenceTearAuthoringSliceTests` 23); iOS build PASS; macOS build PASS. Release-configuration tests were not attempted and cannot run: `ENABLE_TESTABILITY` is Debug-only and `DVSLiveLogger`/`DVSLogEntry`/`TimecodeCMSampleBufferAdapter`/`DVSControlProcessingLoop` are `#if DEBUG`-gated while their tests are not — a pre-existing project limit, not a defect of this slice.

HARDWARE VALIDATION WAS NOT RUN and is deferred to Prompt D, after Prompt C. No RANE ONE MKII take was recorded, no app was launched, no calibration performed, no device unplugged. Production Takes 001–008 and the isolated preview-smoke container are unchanged; no Take009 exists in session `41949897-5458-449d-9280-65508a4f6600`.

The build product at `~/Library/Developer/Xcode/DerivedData/Build/Products/Debug/ScratchLab.app` is NOT an isolated hardware candidate and must not be described as one. Its bundle id is the production `com.machelpnz.scratchlab`, so it resolves to the production container holding the protected takes; the path is shared by every worktree and was overwritten twice on 2026-09-06 (22:19 from the wrong baseline, 22:33 from this worktree). Prompt D requires a fresh build with a unique bundle id and its own container, following the `com.machelpnz.scratchlab.previewsmoke.*` precedent.

Prior entries below are historical and are not rewritten.

# 2026-09-06 — Prompt A clock-boundary follow-up (uncommitted, focused software verification)

Selected Karl's explicit remaining clock-boundary repair in `/private/tmp/scratchlab-motion-provenance-20260906/worktree`, branch `fix/rane-motion-timing-provenance`, unchanged HEAD/base `63e28981c6353ff51d859ef1490e852a94e96343`. The original 14-file candidate and empty index matched the saved diff/source hashes before editing. Primary, TTM, camera-repair and every other existing worktree matched the previous read-only review; the three historical repair baselines and all 108 protected files also matched.

Root cause: the reversal guard checked travel spans but omitted the transition between them. A timestamp jump confined to 0.30–0.31 seconds therefore left the interval unknown while incorrectly confirming a reversal across it. The guard now also checks clock discontinuities against the closed transition span, including a zero-width shared endpoint. Existing non-clock provenance rules and thresholds are unchanged.

This follow-up modifies only `ScratchLab/Models/Reference/ReferenceTake.swift`, `ScratchLabDesktopTests/CaptureReliabilityPhase1Tests.swift`, and the four existing workflow records (AI_HANDOFF.md, AI_HANDOFF/next_prompt.md, DEV_LOG.md, TASKS.md). The full candidate remains fourteen tracked modified files, with no new repository file, project/membership, dependency, renderer, decoder, normalizer, coordinate, fader, export or live UI wiring change.

The new existing-suite test exercises forward 0.00–0.30 → timestamp jump across 0.30–0.31 → backward 0.31–0.61, plus the zero-width 0.30 boundary. It asserts preserved travel endpoints, explicit clock/unknown provenance, zero confirmed reversals and zero Tear holds. `clock-boundary-focused-verified.xcresult` passes 21 tests in EACH of Test Scheme Action and Configuration 2 (42 executions), zero failures/skips/runtime warnings. The same focused suite preserves audited-gap, same/opposite packet-gap, disconnect, wrap, high-displacement reversal, bounded hold, chatter, insufficient-sampling and Baby regressions. The earlier 184-per-configuration evidence is historical and was not rerun after this follow-up.

macOS Debug arm64 and unsigned generic iOS Debug compile gates PASS, with the existing Watch build dependency only. No full gate or separate Watch/Release gate was run, as explicitly requested. The inherited iOS CrossfaderCalibrationStore.swift:67 FileManager/Sendable warning and AppIntents metadata warning remain. Initial sandboxed focused attempt failed before tests ran because Xcode service/CAS/compiler-plugin access was denied (exit 133); its log/result is retained. The authorized retry used the same selector and isolated paths outside the sandbox. Reading its summary also required Xcode TestReport cache permission. These are environment failures, not passing-test evidence.
...
# 2026-09-06 — Rane motion timing/provenance repair (uncommitted, software verified)

Selected exactly Karl's explicit timing/provenance slice, overriding the generic backlog order and full-gate rule. Worktree `/private/tmp/scratchlab-motion-provenance-20260906/worktree`, branch `fix/rane-motion-timing-provenance`, unchanged HEAD/base `63e28981c6353ff51d859ef1490e852a94e96343`, descended from Tear authoring `d0e9ac6` and camera-preview repair. Began clean with an empty index. Dirty primary, TTM and camera-repair worktree files are preserved.

Root cause: the decoder started resumed travel before a packet gap, diluting speed with silence. Filtering/merging then erased opposing-motion provenance, and Reference review treated missing retained events as stationary while absorbing short reversals by duration alone.

Implementation: shared receive-order decoder closes at the last observed pre-gap packet and resets the post-gap counter baseline; repeated zero deltas form explicit stillness evidence, with three packets required. Companion provenance records raw indices, gaps, clock regressions/mismatches, decoder/normalizer discards, insufficient sampling and unknown regions. Normalization cannot bridge controller gaps. Reference artifacts/session/evidence carry the derived companion; Review preserves it and requires continuous measured stillness bounded by same-direction travel for automatic holds. High-displacement short reversals remain directional; bounded low-amplitude chatter remains unknown rather than entering a monotonic curve. The original guard covered clock-conflicted travel spans but missed transition-only discontinuities; the Prompt A follow-up corrects that omission. Fader evidence never supplies structure. Historical movement events and raw packets remain verbatim; the companion is rebuilt on read through the existing decoder, not a new persisted/export schema. Old lost travel is explicitly marked discarded/unknown rather than rewriting historical evidence.

Files: six production files — `ScratchLab/Models/CaptureCore.swift`, `ScratchLab/Models/Reference/ReferenceAuthoringSession.swift`, `ScratchLab/Models/Reference/ReferenceTake.swift`, `ScratchLab/Models/Reference/ReferenceValidation.swift`, `ScratchLabDesktop/Services/MacCaptureEngine.swift`, `ScratchLabDesktop/Services/ReferenceAuthoringCaptureBridge.swift`. Four existing test files — `CaptureReliabilityPhase1Tests.swift`, `ReferenceAuthoringSessionTests.swift`, `ReferenceAuthoringTests.swift`, `ReferenceAuthoringViewModelTests.swift`. Four required workflow records. No new source/test file, membership, project, scheme, renderer, coordinate conversion, export schema, fader lifecycle, signing-file or resource changes.

Verification: final `verified-focused.xcresult`: 184 tests in EACH of `Test Scheme Action` and `Configuration 2`, 368 executions, zero failures/skips/runtime warnings. Twenty new deterministic Rane provenance tests cover all twelve requested shapes plus source/clock/leading/trailing/live/normalizer/25-run cases. Eleven selected suites (eight complete suites plus exact normalizer/live methods) only. macOS Debug arm64 compile PASS; unsigned generic iOS Debug compile PASS (including its existing Watch dependency). No separate Watch or Release gate, no `scripts/build.sh`/full gate: explicitly deferred by Karl. iOS retains an inherited CrossfaderCalibrationStore.swift:67 FileManager/Sendable warning (unchanged source), plus AppIntents metadata notes. No new warning in repaired code.

Initial focused evidence is retained: 156 tests/configuration, three failed test cases/four assertions per configuration. Two inherited tests assumed gap-merging or absorbed equal-counter intervals; the new audited fixture's 30-step pre-gap travel fell below the unchanged normalizer speed floor. Fixtures were corrected to express the intended independent case; no production threshold was tuned to make that fixture pass. The resumed 0.493-second travel already survived. Intermediate 182/configuration passed before the final stillness-reason and clock-reversal audit refinements; final 184/configuration passed afterward. Exact commands, logs, xcresults and final source hashes: `/private/tmp/scratchlab-motion-provenance-20260906/`.

Protection: 65 production session files for `41949897-5458-449d-9280-65508a4f6600` (Takes 001–008) and all 43 isolated preview-smoke container files match the saved historical baselines and this slice's before/after SHA-256 manifest. Manifest digest `a74ae9e9c3c7e8d40ca88acc778807db62bbc982f58426112cb08a02a171e5fd`. The isolated sidecar raw stream was read only to reconfirm 26,385 CC6 packets and the exact 16.385179208344198 → 34.130196916667046 gap; it is diagnostic evidence, not a labeled/training fixture. No hardware interaction, interactive app launch, recording, real approval, export, installation, publication, training, staging, commit or push.

Limits: silence cannot distinguish stillness from disconnect/reconnect because raw packets have no connection-generation record. Gap/clock tolerance 0.10 s, decoder 0.08 s/8 steps/three-packet sampling, normalized motion floors, review 0.25 s AND 0.015 normalized excursion chatter bound, and 0.12 s hold-ambiguity policy still need independent hardware calibration. No physical Tear acceptance is claimed. Coordinate-contract, parked-fader generation, calibration copy and detector advisory defects from the audit remain separate unauthorized slices. Safe next action: read-only review of this candidate and preservation receipts; no new capture or checkpoint action without a new explicit instruction.
...
## Current continuation - Tear authoring / calibration reuse / parked-fader evidence / raw export (checkpoint 2 of 2, UNCOMMITTED candidate, 2026-09-06)

Prepared as an isolated candidate at `/private/tmp/scratchlab-cp2-tear-authoring.sh87xI/worktree`, branch `checkpoint/tear-authoring-raw-export`, base `997bc332f885532ea05b9aac7f636b284785a596` (checkpoint 1, which repaired the non-building HEAD). NOT staged, committed or pushed. The primary worktree `/Users/karlwatson/Downloads/ScratchLab-TTM` is untouched by this candidate and still carries its own dirty copy of the same repair.

EVIDENCE PROVENANCE. Take 007 user-observed screenshots exposed the authoring/review problem. The repair's deterministic verification uses synthetic fixtures; no physical artifact was parsed or modified in this slice. The screenshots are definitively **Take 007** (their finalized-take paths end in `41949897-...._take007_routine`), the same take whose 83 movement events decoded into 81 gestures / 80 reversals and motivated `fba3d86`. **Take 008 also exists** as a separate real artifact in the sandboxed production container `~/Library/Containers/com.machelpnz.scratchlab/Data/Library/Application Support/ScratchLab/`: 55 movement events, 19,097 mixer MIDI events, zero mapped crossfader samples, linked Watch capture `786056BA-4C08-4277-9422-43FE0BF88E2D`, completed/manual stop. An earlier audit searched the NON-container path `~/Library/Application Support/ScratchLab/` and wrongly concluded Take 008 did not exist; the 202-file manifest from that path does NOT protect the real sandboxed evidence. Neither take was parsed or modified by this work, and neither is approved, published, installed or training-eligible. Scope is **15 implementation/test files plus four workflow documents**, not the "19 files" earlier drafts claimed.

Boundary. Production (9): `CaptureCore.swift` (+161/-0 in exactly four hunks: type +151, sidecar field/doc +8, init parameter +1, init assignment +1), `ReferenceTechnique.swift`, `ReferenceTake.swift`, `ReferenceValidation.swift`, `ReferenceAuthoringSession.swift`, `MacCaptureEngine.swift`, `ReferenceAuthoringCaptureBridge.swift`, `ReferenceAuthoringViewModel.swift`, `ReferenceAuthoringView.swift`. Tests (6): the four `ReferenceAuthoring*` suites plus `LivePerformedNotationTrackerTests` and `MacCameraPreviewViewTests`. Excluded and left dirty in the primary: `project.pbxproj` (71/71 pure reordering) and `CrossfaderCalibrationStore.swift` (3/5).

Source guards were made precise, not deleted. The blanket ban on naming the shared chart named the SHARED renderer this repair correctly routes through, and the blanket Canvas ban was already violated at committed HEAD by `TearReviewTimelineChart` (a3d86e9). A new test `testThePreExistingTimelineCanvasIsPinnedAndNotWidened` pins that Canvas by count and by owning type, so the pre-existing violation is identified rather than legitimized and any new hand-rolled renderer fails. All capture/approval/publication bans are unchanged.

Verification, isolated: unique signed host `com.machelpnz.scratchlab.cp2.sh87xi` with its own sandbox container, isolated SYMROOT/OBJROOT/DerivedData/products/result bundles, no bare `xcrun xctest`, no concurrent `xcodebuild`, candidate tree frozen (identical diff digest before and after). Python 82 OK; full XCTest plan 3,729 executed / 56 skipped / **0 failures** both configurations; focused suites 778 executed / **0 failures**; iOS, macOS, watchOS builds PASS. Production Application Support store: **zero files written** during the run, while 186 landed in the candidate's own container.

The literal `scripts/build.sh all` was NOT run: `xcodebuild` ignores SYMROOT/OBJROOT from the environment on this Mac (`IDEBuildLocationStyle = Custom`), so the script cannot be given an isolated container or products. Its five stages were run individually with stricter isolation instead.

REMAINING RISK: **no hardware verification.** Nothing has been recorded on the Rane through this code, the canonical tear chart has not been seen in a running app, and the parked-fader take-start path has never executed against real Core MIDI. No reference is valid, approved, published, installed or training-eligible. No package round-trip was exercised.

Next: review the candidate, then either commit it there or stage the same boundary in the primary, exactly as checkpoint 1 was landed.
...
This record accompanies the single authorized checkpoint `feat(notation): render canonical tear motion and fader evidence`, parent `366f4c250f30db445210809f93062dd0898d892e`, on `feature/ttm-tear-notation-alignment` in `/Users/karlwatson/Downloads/ScratchLab-TTM`. The user's checkpoint request supersedes the implementation turn's no-commit instruction for this one isolated commit. Implementation began with a clean worktree/index; all eleven complete diffs contain only this slice. After successful commit, expect a clean worktree/index. No push or next implementation slice is authorized.

The existing shared chart now accepts `.canonical(records, layer, frame)` and renders through the same `ScratchStrokeGeometry` / `MotionPath` / `ScratchMotionRenderer`. The explicit frame shares seconds, BPM and position units/range across target/performance. Curves retain actual slopes; observed holds are horizontal; fader-only edges mint glyphs; closed motion/ghost-holds use dim dashed bone/cyan; missing motion/fader is explicit. V3.2 separate platter/fader lanes and legacy entry points remain. No detector/navigation/export/training or capture-producer changes.

Seven implementation/test files plus four workflow records form this slice; see the final Prompt 4 DEV_LOG entries. At checkpoint, all seven source/test SHA-256 hashes match the final verified build and all 11 protected project/scheme/test-plan/plist/privacy/entitlement files are byte-identical to the parent. Audits: `build/prompt4/audit.json` and `checkpoint-audit.json`; complete pre-checkpoint diff: `checkpoint-before-docs.diff`. Final macOS arm64 test build passed (`build-regression.xcresult`). Exact focused 18-suite selection passed 180 declarations / 398 expanded executions in both configurations, including all 23 new renderer tests twice; zero failures/skips/runtime warnings. Saved bundles were re-read at checkpoint and match the original summaries: `checkpoint-focused-summary.json`, `checkpoint-full-summary.json`. Exact commands: `build/prompt4/commands.txt`. No source/test changes or test reruns were needed for the documentation-only checkpoint. Earlier failed/superseded evidence is retained.

Full gate: PASS: Python 82; native 8,178 expanded passes / 112 skips / 0 failures (4,089 passes and 56 skips per configuration); two main-thread UI-unresponsiveness runtime warnings; iOS, macOS and watchOS builds all ran and passed (iOS/watchOS unsigned). `scripts/build.sh all` exited 0; use `full-gate-rerun.log`, `full-tests-rerun.xcresult`, and `full-final-summary.json` under `build/prompt4`. No raw capture was deleted or rewritten; no recording or bundled reference was made valid, canonical, approved or training-eligible; no detector inference was presented as human ground truth; no ScratchBook GPL source/code/data was copied; no unrelated work is included in the checkpoint. No physical CXL, GPU bitmap snapshot, deployment or training acceptance is claimed. TASKS is complete. The exact next safe numbered task is **Prompt 5 — read-only preflight and specification check only**, in `AI_HANDOFF/next_prompt.md`; no Prompt 5 implementation brief has been supplied.
...
- Boundary 1 commit subject: `fix(capture): expose routine timing and audio lookup in Release`. Five files: `ScratchLabDesktop/Services/MacCaptureEngine.swift`, `TASKS.md`, `DEV_LOG.md`, `AI_HANDOFF.md`, `AI_HANDOFF/next_prompt.md`.
- Exactly four audited engine hunks (old anchors 4004, 4167, 5368, 5853): two conditional-compilation directives expose existing routine media-epoch/timed-stop members outside DEBUG, and the 47-line `audioDeviceID(forUID:)` implementation moves outside ENABLE_TIMECODE_LIVE_TAP with its payload verified byte-identical. Engine diff +49/-47. Nothing removed from DEBUG, so Debug behaviour is unchanged. No test, project-membership, scheme, entitlement, plist or build-number change; build remains 21.
- Focused: 45 per configuration, 90 total, zero skipped, zero failed. Serial `scripts/build.sh all` exit 0: Python 82; XCTest 3,335 cases per configuration, 55 skipped, 0 failed (6,670 executions, 110 skipped); Swift Testing 366 in 49 suites; iOS, macOS and unsigned watchOS builds passed. Unsigned universal macOS Release passed, `lipo` reports x86_64 arm64, build 21. Ten compiler warnings remain unrepaired.
- Karl authorized treating only `MIDILearnEngineTests/testCalibrationIsolatedBetweenDevices` failing with `Optional(0)` vs `Optional(5)` at `MIDILearnEngineTests.swift:561` as a known non-blocking load-sensitive flake, and forbade modifying the test. It was not modified, and it passed both configurations in the committed gate run, so the allowance was unused. The earlier failing log and bundle are retained.
- Extraction tooling: `apply-hunks.rb` applies selected hunks by OLD-side coordinates only. `git apply --unidiff-zero` must not be used for subset extraction here, because pure-insertion hunks carry new-side line numbers inflated by other boundaries and place content in the wrong location. Boundaries 3 and 5 additionally require sub-hunk splitting of engine hunks 10790 and 10812.
- Verified decomposition for the remaining boundaries: the 39 engine hunks partition as B1 four, B3 six, B5 twenty-nine. All eight `CaptureCore.swift` hunks belong to Boundary 3, so CaptureCore becomes clean after that commit. All four `MacAnalyzerView.swift` hunks belong to Boundary 6. `SessionExportCoordinator.swift` hunk old 911 is the self-contained helper at new lines 912-1029; the other 21 hunks stay excluded. `project.pbxproj` holds 14 membership hunks plus exactly six excluded build-number hunks, and membership lines are interleaved so they must be split per boundary at line level, not hunk level. `CaptureReliabilityPhase1Tests.swift` is identical to the primary base through line 22196 with the 290-line receipt block inserted there, so old-side coordinates align for hunks 3202, 3204, 12009 and 14151.
- Isolation: unique signed sandbox host `com.machelpnz.scratchlab.refauth-iymlzu`, separate Test/Build/Release lanes, guarded host and xctestrun identities, production-denying Python sandbox. Logs and result bundles live under `../logs` and `../results`. Preserve all outputs.
- All 661 primary files and all 93 protected evidence hashes match the saved baselines. Primary HEAD, branch and index bytes are unchanged; receipt branch intact; protected settings and build numbers unchanged; `git diff --check` passes. Takes 001-006 and historical metadata mutations are untouched.
- Not done and not authorized here: push, merge, cherry-pick, deployment, ordinary app launch, Take 007, reference approval, package installation or publication, training, and TTM/Tear. TTM Prompt 1 remains blocked until the Boundary 3 CaptureCore checkpoint exists.

## 2026-09-05 - Current receipt candidate verification PASS; no commit

This validation-only update supersedes the preparation status retained below. Worktree /private/tmp/scratchlab-receipt-candidate.Bs1CNx/worktree remains detached at d3ecf2a69bde57f7d6ac681b6dbdd37347c83e97. Only the four candidate receipt documentation entries changed during validation; production/test code and the external isolation wrapper are unchanged.

- Original SIGTERM investigation: archived Dock/libquit force-quit request for isolated host PID 89618 at 11:32:22.330, then SIGTERM(15), about 0.57 s after the named test started. Best-supported immediate cause is external Dock-mediated termination; initiating actor and preceding brief UI unresponsiveness are unresolved. No assertion/exception/test-timeout or host memory-kill evidence. Test-manager cancellation followed exit. Original failed full.xcresult and its full-build-gate.log remain intact; details/excerpt are in the final DEV_LOG entry and ../sigterm-diagnostics / ../sigterm-system-excerpt.log.
- Controlled selector: testMappedCrossfaderMIDIRoundTripsExportWithFaderEvents alone passed initially in configuration 1, three more times in configuration 1, then once in Configuration 2 (each exactly 1 passed / 0 failed / 0 skipped). Fresh logs/bundles: ../sigterm-c1-initial, ../sigterm-c1-repeat1, ../sigterm-c1-repeat2, ../sigterm-c1-repeat3, ../sigterm-c2 with .log/.xcresult suffixes. No code/test workaround, skip or selector change.
- Single authorized fresh scripts/build.sh all: PASS, exit 0; Python 82/82; 3,701 unique native cases (3,646 passed / 55 skipped / 0 failed), 7,430 expanded executions (7,320 passed / 110 skipped / 0 failed). Both configurations passed, including 18 receipt staging tests each and the formerly terminated test; two main-thread runtime warnings remain. iOS, standalone macOS and watchOS build phases all ran and passed. Evidence: ../full-sigterm-validation.log, ../full-sigterm-validation.xcresult and ../full-sigterm-validation-summary.json. Historical focused 244-pass result is also reusable for unchanged code.
- Commit readiness: audited seven-file receipt candidate is software-verified and ready for a separately approved narrow commit, not staged or committed. Both HEADs/indexes, primary 661-file baseline and all 93 protected evidence hashes remain unchanged. Project/schemes/entitlements/Info.plist/build numbers remain unchanged. Candidate remains build 21 and has NOT been deployed or hardware-tested; Take 006 build-22 evidence and its zero-fader limitation are historical, not candidate hardware proof. Preserve all historical metadata mutations and separate risks below; no TTM, Take 007, deployment, approval, publication or training action.
- Future testing safety: the successful standalone macOS build leaves the normal bundle identity in isolated Products/Debug. Do not launch it or bypass the test-host guard. Any newly authorized tests must first rebuild the isolated test host with the established build-for-testing override and verify its unique signed sandbox/container before executing tests. No further test/build is required solely for this unchanged validated candidate.

## 2026-09-05 - Historical preparation: focused PASS, original full gate BLOCKED, no commit

At preparation time this entry governed the candidate; it is retained as the original result, superseded by the current validation update above. Historical HEAD records remain intact. Karl authorized preparing and verifying the receipt-only candidate, not staging, committing, deployment or recording.

- Base: d3ecf2a69bde57f7d6ac681b6dbdd37347c83e97. Detached worktree: /private/tmp/scratchlab-receipt-candidate.Bs1CNx/worktree. Primary worktree is untouched and remains mixed/dirty.
- Root cause: WCSessionFile.fileURL expired when the delegate returned before async copying. Receipt now moves synchronously into Application Support/ScratchLab/IncomingWatchTransfers; async work receives only the stable URL. Durable import verifies content, preserves exact identities, does not overwrite existing captures, cleans up only its successful receipt, and retains failures/diagnostics for deterministic startup recovery.
- Candidate code: complete receipt-only diffs in WatchMotionCaptureStore.swift and StagedCaptureRecovery.swift plus ONLY the 290-line WatchMotionCaptureStoreStagingTests insertion in CaptureReliabilityPhase1Tests.swift (18 tests). Earlier source-assertion, DEBUG-route and crossfader tests are excluded. No project, scheme, build-number or membership changes; dependencies already exist at HEAD.
- Fresh candidate software verification: native build-for-testing PASS; exact focused selection PASS, 122 per configuration / 244 total, zero failures/skips, staging 18 per configuration. Serial scripts/build.sh all: Python 82 PASS, macOS full test plan FAIL / exit 65. Bundle: 3,701 unique cases (3,645 passed, 55 skipped, 1 failed); 7,430 parameter-expanded runs (7,319 passed, 110 skipped, 1 failed). Existing testMappedCrossfaderMIDIRoundTripsExportWithFaderEvents reports "Test crashed with signal term" in Test Scheme Action; Configuration 2 passed unchanged in 2.709 s. Its body and export implementation match HEAD. Termination cause is not established and is not attributed to the receipt repair or the separate documented SPSC flake. No manual retry or repair occurred. Script stopped before iOS/standalone macOS/watchOS builds: NOT RUN. Candidate is NOT commit-ready.
- Isolation: actual signed host and both xctestrun configurations use com.machelpnz.scratchlab.receipt-candidate.bs1cnx, with App Sandbox enabled, no shared-group or filesystem write exceptions and a fresh separate container. XCTest injects a root read-only exception; do not claim absolute read isolation. Storage roots and test startup guards were inspected; no production capture path is supplied. Products, intermediates, DerivedData, module caches and result bundles are under /private/tmp/scratchlab-receipt-candidate.Bs1CNx. Python fixtures run under an additional production-denying sandbox. An initial build-only outer-sandbox experiment blocked nested macro plug-ins; native build-only retry passed without source changes. Full commands, caveats and results are in the final DEV_LOG entry; focused.xcresult and full.xcresult are retained beside the worktree.
- Historical hardware evidence ONLY: Take 006 on primary Mac/iPhone 1.0.1 (22), unchanged Watch 1.0.1 (21), session 41949897-5458-449d-9280-65508a4f6600, take-006, capture D3687AE4-9476-40AD-BB00-0A7AE6DE29BC. Matching durable phone quarantine bytes, Mac motion, reconciliation and sidecar links; 835 ordered samples, about 2 s to phone and 5 s to link. Transient staging was consumed before inspection, not directly observed. Full hardware checklist did NOT pass: zero take-scoped fader samples. This isolated candidate has NOT been hardware-tested or deployed and retains HEAD build 21.
- Preserve Takes 001-006 and all earlier metadata mutations from the test-host isolation failure and normal relay-summary refreshes; do not restore them. Missing-transfer Takes 003/004/005 are not backfilled. No raw capture deletion/rewrite, approval/canonicalization, publication, reference installation, training enablement, detector-as-ground-truth claim or ScratchBook GPL copying is permitted.
- Separate unresolved risks: final live-batch identity ordering, quarantined reconnect retries, Mac acknowledgement before proven retention, stale nested transfer state, D5 session reuse and missing fresh in-take OPEN fader evidence. D6 hardware evidence belongs to the primary layout slice, not this candidate. TTM/Tear remains out of scope.
- Final preservation: all 661 primary tracked/untracked regular files match the audit baseline, none added/missing; both indexes empty and HEAD unchanged. All 93 protected evidence files match, including retained historical metadata mutations. Candidate project/schemes/entitlements/Info.plist match HEAD, app build numbers remain 21, and git diff --check passes. Only seven candidate files differ. Baselines and logs remain beside the worktree; no cleanup, staging, commit, push, deployment, recording, approval or training occurred.

## 2026-09-04 (latest) - Watch stop DONE and hardware-verified; ASC plist blockers fixed; nothing committed

- **Watch stop is fixed and proven.** `session_2026_09_04_ppop_baby_scratch_95_bpm`: validator **PASS, exit 0, zero warnings, zero errors**. `watchSyncState: acknowledged`, `watchStopOutcome: stopped`, `watchMotionTransferState: completed`, overrun **0 s**, lead-in 3.000 s. All four handshake instants land in the same second (04:16:47Z), so the full Mac -> relay -> watch -> back round trip completed inside one second, against a full 4.0 s timeout returning nothing on earlier runs.
- **Root cause was not latency.** `WatchMotionCaptureStore.requestRemoteCaptureStart/Stop` default `commandID` to a fresh `UUID()`, and `ScratchLabApp.handleRemoteWatchControlCommand` called them without forwarding the Mac's ID. The watch replied under an ID the Mac never issued, `WatchCaptureCommandCoordinator.resolve` found nothing pending, and the await was never resumed - so both handshakes timed out unconditionally however fast the watch answered. Fixed by carrying `commandID: payload.commandID` through.
- Also landed, in the same investigation: fail-closed stop ownership (`CaptureWatchStopPolicy.startMayHaveLeftWatchRecording`); stop dispatched exactly once from every terminal Mac path; identity-scoped, idempotent watch stop handler; overrun measured between END instants rather than durations (`takeStartedAt`, `takeStopRequestedAt`, `watchCaptureStartedAt`, `watchCaptureEndedAt`); a relay `.requested` receipt so a future timeout says whether the phone or the watch is at fault; control commands delivered by a direct closure instead of a SwiftUI `.onChange`; and two real serializations removed from the reply path (watch replying only after writing its motion file, iPhone relaying acks on the camera's sample-buffer queue) - genuine improvements, but NOT the cause, and the passing run credits nothing to them.
- **ASC `Info.plist` blockers fixed with Karl's explicit approval** (`SOUL.md` otherwise gates these): macOS `CFBundleVersion` `20` -> `$(CURRENT_PROJECT_VERSION)`; `NSAudioCaptureUsageDescription` added for the `AudioHardwareCreateProcessTap` Direct Capture path; 11 non-existent watch icon filenames removed from `CFBundleIconFiles`; invalid `LSApplicationCategory` removed. Four regressions guard them. No build settings, entitlements, signing, bundle IDs or Copy Bundle Resources were touched.
- **Nothing is committed or pushed.** 20 modified files, all listed in the DEV_LOG entries for 2026-09-04.
- Known-open, unchanged: `TimecodeRealtimeIngressRingTests.testConcurrentSPSCStressAccountsForEveryAttempt` is flaky and makes `scripts/build.sh` non-deterministic (it passed config 1 and failed config 2 of a single earlier run; passes 10/10 standalone). `ScratchLabWatch` still has no `PrivacyInfo.xcprivacy` - needs a `project.pbxproj` edit, so it is a separate approval, and is not a blocker today because nothing in that target uses a required-reason API.


## 2026-09-04 (latest) - Watch stop CONFIRMED FIXED on hardware; relay ack latency is the open defect

- Two hardware runs. Run 1 (`4ee15cc0-…`) still failed: `watchSyncState: failed`, `watchStopOutcome: notRequested`, no stop dispatched. Cause was a defect in the first fix - stop-ownership was gated on `reply.syncState == .acknowledged`, which is fail-open, because a degraded *reply* is not evidence the Watch did not start. Fixed with `CaptureWatchStopPolicy.startMayHaveLeftWatchRecording(_:)`: `acknowledged`, `requested`, `failed` and `timedOut` all arm the stop; only `unavailable` and `notRequested` own nothing.
- Run 2 (`1ce25396-…`, exported as `session_2026_09_04_kjhgt_baby_scratch_95_bpm`): **Karl confirmed the Watch stopped by itself.** Timeline, UTC: start sent 03:39:14, Watch began 03:39:15, Mac gave up on the start ack 03:39:17 (`failed`), media began 03:39:20, Stop pressed and stop dispatched 03:39:30, **Watch ended 03:39:30**, link adopted at finalization 03:39:33, stop resolved `timedOut` 03:39:35. The Watch stopped in the same second it was asked. The fail-closed fix is what dispatched that stop, since the start had come back `failed`.
- **The validator was measuring the wrong quantity and false-failed a good take.** It compared durations and called the difference "past the end of the take", but the Watch's window and the take's window do not share a start. Run 2's 5.411 s was entirely lead-in. The export now carries `takeStartedAt`, `takeStopRequestedAt`, `watchCaptureStartedAt`, `watchCaptureEndedAt`; overrun is now `watchCaptureEndedAt - takeStopRequestedAt` (warn 2.0 s, error 4.0 s) and lead-in is reported separately (warn 3.0 s). An archive without those instants gets an honest "cannot tell a late stop from an early start" warning, never an overrun error. The earlier "18.494 s past the end of the take" claim about BVB was stated more confidently than that archive supported.
- **Open defect, next action: the relay's ack round-trip is about 5 s in both directions.** The Watch acts immediately; the acknowledgement does not get back inside the 3 s start bound or the 4 s stop bound. Every take therefore records a spurious `failed` start and `timedOut` stop, and pays ~3 s of needless lead-in from the blocking start await. Find where the time goes before touching any timeout - widening bounds without the cause would hide it. Measure WCSession reachability when the Watch screen sleeps mid-take, the iPhone relay's SwiftUI `onChange` dispatch, and MultipeerConnectivity send latency.
- All three targets are built and installed from this tree: macOS `DerivedData/Build/Products/Debug/ScratchLab.app`, iPhone `K`, and the paired Series 10 Watch. Nothing is committed.
- Verification: capture-pipeline fixtures 82/82; macOS watch-stop suites 36/36; macOS build-for-testing, macOS build and iOS device build all succeeded. The full `scripts/build.sh all` gate last exited 65 on the pre-existing `TimecodeRealtimeIngressRingTests.testConcurrentSPSCStressAccountsForEveryAttempt` flake, which is tracked in `TASKS.md` and is unrelated to this work.


## 2026-09-04 (latest) - Mac Stop now ends the Watch capture; hardware proof outstanding

- Defect, from `session_2026_09_04_bvb_baby_scratch_95_bpm`: a 19.4 s macOS take exported clean by every existing check (`stopReason: manual`, 19.235624 s camera, three stems at exactly 855,540 frames, `watch_source: watch`, Watch CSV present and linked, validator PASS) while that CSV ran 0.0 to 37.8943409166659 s. The Watch recorded 18.4943409166659 s past the take and Karl stopped it by hand. The earlier artifact-association/write-back fix is working and was not regressed - the motion reached the Mac, was linked, and exported.
- Root cause had three parts, all in the Mac's stop path. (1) Ownership: the Watch stop lived in `MacAnalyzerView.handleRoutineRecordingButton`, so only the Stop *button* sent one; the timed backstop, AVFoundation's own `maxRecordedDuration` end, capture errors, refused starts and cancelled count-ins sent nothing. (2) Identity: that call passed `routineSessionSetup.config.sessionID` with `takeID: nil`, not the `TakeIdentity` the start was acknowledged under. (3) No acknowledgement: `requestWatchCaptureStop` returned `Void`, reported only a local send failure, never awaited, retried, or recorded anything - so any downstream failure was invisible and the take still exported as if the Watch had stopped.
- Which of the three silenced this particular capture is not decidable from the archive, because nothing recorded it. That absence is itself the defect; the new diagnostics are the fix for it.
- `MacCaptureEngine` now owns the stop. `applyPendingWatchReply` grants ownership only on an `acknowledged` start with a complete identity; `requestWatchStopIfNeeded(reason:)` dispatches exactly once per take keyed `sessionID:takeID` and is called from `stopRoutineRecording`, `finalizeRoutineRecording`, `cancelPendingRoutineReservation`, and every abandoned-start path. It returns immediately - the bounded handshake (2.0 s per attempt, 2 attempts, 4.0 s ceiling in `CaptureWatchStopPolicy`) runs on a task and its outcome is merged into the take's on-disk sidecar. `persistWatchStopDiagnostics` re-reads the file and writes only the stop-diagnostics field, so `mergingLatestWatchAssociation` is preserved.
- The Watch handler is one `resolveControlCommand` behind both `didReceiveMessage` overloads, driven by the pure `WatchMotionStopCommandResolver`: idempotent on a repeated command or an idle recorder, and it refuses a stop naming a session or take it is not recording. A handled stop still answers `syncState: notRequested` (the shipped contract `withWatchSync` depends on) and adds the new `stopOutcome`.
- Exported diagnostics, additive and app-side only: `watchStopOutcome` (`notRequested` / `sent` / `unreachable` / `timedOut` / `identityRejected` / `stopped` / `failed`), `watchStopDetail`, and `watchMotionTransferState` (`notApplicable` / `pending` / `completed`). `watch_source` stays two-valued and `watchSyncState` keeps meaning the *start* handshake - neither is overloaded.
- `validate_session.py` now compares the Watch CSV's largest `elapsed_time` against the take's canonical audio duration: over 2.0 s warns, over 4.0 s errors. Run against the real BVB archive it reports `95 BPM take 01: watch motion runs 37.894s against a 19.400s take - 18.494s past the end of the take (max 4.000s)`, Status FAIL, exit 1. The archive was not modified and the raw CSV is never truncated.
- **Physical proof still required and is the next action.** Record one fresh Mac + Watch take, press Stop on the Mac only, never touch the Watch. Confirm: the Watch stops on its own; the Watch CSV's largest `elapsed_time` is within 4.0 s of the take audio; `watchStopOutcome` reads `stopped` in `manifests/session_metadata.json`; `watchMotionTransferState` reads `completed`; the validator passes with no watch-overrun warning. Then, once each: a take stopped with the Watch out of range (expect `unreachable`/`timedOut` and a visible degraded status, never silent success) and a cancelled count-in (expect the Watch to stop on its own).
- Gate: `scripts/build.sh all` exited **65** on a pre-existing flake, not this work. Fixtures 79/79. The macOS test plan's configuration 1 passed 3304/3304 (51 skipped, 0 failures); configuration 2 failed only `TimecodeRealtimeIngressRingTests.testConcurrentSPSCStressAccountsForEveryAttempt`, which had passed in configuration 1 of the same run and passes 10/10 standalone. `TimecodeRealtimeIngressRing` is untouched by this slice. It is a genuine flaky lock-free ring test and needs its own slice. iOS, macOS and watchOS builds were run directly afterwards on the same tree and all three succeeded.
- An App Store Connect readiness audit was also run in the same session; its findings are in the session report and are not yet actioned in the tree. The two confirmed repository blockers are the macOS `CFBundleVersion` literal `20` in `ScratchLabDesktop/Info.plist` against `CURRENT_PROJECT_VERSION = 21`, and the missing `NSAudioCaptureUsageDescription` for the `AudioHardwareCreateProcessTap` Direct Capture path. Both are Info.plist edits, which `SOUL.md` puts behind explicit approval, so neither was applied.
...
# Current handoff — CXL operator Slice 1 complete

Work only in `/private/tmp/scratchlab-cxl-operator-20260908.HS06xS/candidate` on branch `codex/cxl-operator-release`. Base is `3f43345a6c6369a4b18bbb52134a1666ac505b30`; the exact completed TTM Prompt 9–12 patch was inherited before this slice. Preserve the dirty primary checkout, the completed TTM candidate, and every prior evidence root.

The formal CXL roadmap, evidence board, and 32-row/minimum-40-variant run sheets are now in `docs/`. `AI_CONTEXT.md` corrects plain Tear semantics so fader evidence is independent unless a named variant requires it. These records make no capture or acceptance claim.

Selected implementation task: explicit `Next take` after canonical approval. Session, serial worker, view model, approved-state UI, and six behavioral tests are implemented. The transition requires the latest approved take ID, changes only `phase` to `readyToRecord`, retains all approved records and setup, and never starts recording. The UI blocks during other work, Watch transfer, or export preparation and clears only transient review notes after a successful transition.

The first frozen prepare gate is a retained compile failure caused by one new test referencing `takeNumber` on the take wrapper. The actual persisted field is `evidence.metadata.takeNumber`; only that assertion was corrected. Production compiled in the failed attempt.

`cxl-prepare2` passed. Contract review then found missing deterministic busy and duplicate/stale queue coverage in the draft test set. The tests now contain the agreed six behavioral declarations, including phase/identity refusal and four distinct serial approvals; this strengthening changed no production source.

`cxl-prepare3` passed. The first fully scoped focused gate retained one assertion failure in both configurations: 104 unique/208 executions, 206 passed and two failed, zero skipped/warnings. Next-take refusal behaved correctly; the assertion wrongly required a concurrent preflight refresh to preserve diagnostic preflight fields after the refresh completed. It now preserves and compares the actual contracted fields and uses the truthful post-preflight session as the export-refusal baseline. No production change.

`cxl-prepare4` and `cxl-focused2` passed: five required suites, 104 unique/208 executions, all passed with zero skips/warnings. The first full attempt passed Python 82/82 and desktop 4,483 unique/9,238 executions = 9,126 passed/112 skipped/zero failed with two known warnings. The gate then failed source integrity because the three new planning documents changed concurrently to add the versioned, rights-clear CXL Beat Library and revise roadmap ordering. Exact comparison found no production/test change. Preserve those compatible additions. Platform legs did not run.

`cxl-full2` repeated the same clean Python/desktop results and stopped for a second concurrent edit to only those three planning documents. The latest preserved plan caps the pilot at six beat variants and defines a four-day Capture Ready boundary. Final verification separates the runtime/build-input digest from these noncompiled plans and records their hashes independently, so plan drafting cannot invalidate or silently alter the compiled evidence.

`cxl-full3` then failed against 579 concurrent protected production/shared-product changes and two abnormal test-runner terminations in one configuration; the other configuration passed those cases. Nothing was restored. No ScratchLab/Xcode process remained after the stop. Preserve the complete failed evidence and use a fresh verification root/baseline; do not weaken production preservation.

Fresh-baseline verification is complete. Focused Reference Authoring: 104 unique/208 executions, all passed. Exact diagnostics: two unique/four executions, all passed. Final full evidence on runtime digest `dc3044bd111822b38222854363e0e283d6c9aa3fae743fc08a4856c56d309008`: Python 82/82; desktop 4,483 unique/9,238 executions = 9,126 passed/112 skipped/zero failed with two known warnings; iOS and macOS builds passed. Watch passed separately. The full wrapper's preservation guard stopped on external Xcode/indexing mutations to shared products and old-candidate UI metadata; ScratchLab was running and Xcode was indexing the completed TTM candidate. Nothing external was stopped or restored. The runtime source stayed frozen.

Post-test comparison found only the four final workflow records plus ignored current-candidate `xcuserdata` changed by Xcode indexing. Runtime verification excludes that UI metadata; no production/test source changed after the passing results.

This slice is complete. Next reviewed dependency is immutable capture-intent plus exact BeatSpec binding through capture/review/export/reopen, but do not begin it without a new user proceed. The focused Figma audit/design remains before broader operator-layout implementation. No commit, push, hardware use, recording, installation, publication, training change, upload, or cleanup occurred.

---
# 2026-09-08 — Prompt 13 frozen candidate; Prompt 14 next

Prompt 13 is complete in `/private/tmp/scratchlab-cxl-auto-20260908/p13-tear-fader` on `codex/cxl-auto-p13-tear-fader`, inherited base `3f43345a6c6369a4b18bbb52134a1666ac505b30`. The code now uses the physical end rail as closed, mirrors deck/open-end geometry, projects gate state through the same resolved curve as audio, snapshots that curve for deterministic reopen, matches stable MIDI source identity plus channel/CC, adopts only exact pre-take snapshots, and keeps provisional or low-confidence platter evidence fail-closed. Raw calibrated position remains preserved.

Verification: Python 82/82; focused and widened authoring/capture suites pass; the generated-media Tear evidence pipeline and parked-fader authoring suites pass serially in both configurations; iOS/macOS/Watch builds pass; diff check passes. Retain `receipts/p13-tear-fader/full-build.log` as a failed pre-repair full run and the Xcode bootstrap failure as infrastructure evidence. No RANE, launch, install, recording, calibration, approval, publication, upload, or training occurred. Freeze this state before Prompt 14.
# 2026-09-08 — Prompts 13–21 software complete; bounded Rane pilot next

Continue only from `/private/tmp/scratchlab-cxl-auto-20260908/p21-final-gates` on `codex/cxl-auto-p21-final-gates`. The primary checkout and all earlier prompt worktrees remain untouched. The exact unsigned universal candidate is `/private/tmp/scratchlab-cxl-auto-20260908/products-p21-universal-release-candidate/Release/ScratchLab.app`, bundle `com.machelpnz.scratchlab.cxl-authoring`, executable SHA-256 `4d6e19153ab829e238b4df18ca12c1d0d19493a335321b7afefd9ef2e8b9e34a`.

Prompts 13–20 now form one connected CXL authoring path: truthful Tear/fader projection, immutable CaptureIntent/BeatSpec, unapproved procedural beat pilots, witnessed timing, finalized-media review, terminal source state, approved package export/reopen, and direct CXL Release routing. Prompt 21 repaired only gate-proven regressions and tests. Final focused passes are 560 tests/1 skip/0 failures; direct full Debug and Release-optimized passes are 3,973 tests/56 skips/0 failures per configured execution; the final required wrapper passes 3,973 tests/55 skips/0 failures plus all platform builds. The hook-free Release binary is universal and has no checked Debug route/tool/test symbols.

The next action requires Karl and the Rane. Follow `RANE_PHYSICAL_HANDOFF.md` exactly and stop at the first FAIL or BLOCKED result. Do not infer acceptance from software evidence. Do not install for training, publish, upload, train, commit, push, or clean any worktree.

# 2026-09-08 — Hardware-feedback CXL candidate replaces the unsigned P21 app

Continue only from `/private/tmp/scratchlab-cxl-auto-20260908/p21-final-gates`. The primary checkout and every earlier worktree remain untouched. The exact current app is `/private/tmp/scratchlab-cxl-auto-20260908/products-p22c-cxl-hardware-fix/Release/ScratchLab.app`; executable SHA-256 `7c1d4edaba6646b15dcd6cbd9234685661924f9aa0a77667f0710be943989cc9`; bundle `com.machelpnz.scratchlab.cxl-authoring`; architectures `x86_64 arm64`. It has a complete local ad-hoc signature, sealed Info.plist/resources/capture entitlements, and passes strict deep verification. PID 80042 was read back from `ps` with this exact executable. The Mac then remained locked, so no window, prompt, device, or hardware observation is claimed.

Karl's screenshots proved the earlier live Tear chart alternated within one session between calibrated platter revolutions and take-local normalized displacement, producing tiny disconnected arcs, full-height rescaling, and large unknown blocks. CXL now constructs its sole tracker with playback-loop context permanently disabled and does not pass a wrap period to the shared canonical renderer. The qualifying provisional run retains the decoder's `meetsNoiseGates`; packet-gap evidence still renders `MOTION UNKNOWN`; generic Practice/Capture keeps the existing loop-aware default. The established 3.2-second rolling window can still refit when old physical events age out, independently of AHHH playback.

Release now opens with visible Setup/Capture/Review & Export stages and exact MIDI, camera, and audio selectors. Route appearance performs discovery only. Camera and microphone configuration require the explicit camera/audio button; iPhone/Watch relay has a separate off-by-default button; Serato process-tap discovery is disabled and its workspace callback exits immediately. Continuity cameras may appear as choices but are not selected or started automatically. A newly signed local build may receive one camera and one microphone prompt only after the operator explicitly enables the selected devices.

Final focused verification is frozen and passing: 17 unique tests, two configurations, 34/34 executions, zero failures/skips/runtime warnings; 453 Swift/project/test-plan inputs are unchanged, manifest SHA-256 `5a0f72aba42016ac540b3476aae84e5c1d253324fbab0119d323a03a78c30b72`. The universal Release build and strict signature checks pass. The required broad Prompt 13–21 gates remain the previously recorded passing results; `scripts/build.sh all` was not repeated after this narrow hardware-feedback repair.

Next action is only the first Rane ONE MKII diagnostic step in `AI_HANDOFF/next_prompt.md` and `RANE_PHYSICAL_HANDOFF.md`. The intended CXL production path, Rane Seventy-Two plus Rane Twelve connected through the Seventy-Two to the Mac, remains NOT RUN and cannot be accepted from a Rane ONE result. The current Setup selects an audio device but does not yet provide pair-specific proof for that production rig's DVS and master-return routing. All permission, notation, recording, calibration, media, Watch, export, operator, publication, upload, training, commit, push, and cleanup results remain NOT RUN.

## 2026-09-08 — Additional software-only audit while operator away

A frozen `scripts/build.sh all` rerun was attempted after the narrow hardware-feedback repair. Python passed 82/82, then desktop test compilation stopped with `No space left on device` while only about 101 MB remained. No XCTest or platform leg ran, so the attempt is BLOCKED and must not be called a full-gate pass. Its log is `/private/tmp/scratchlab-cxl-auto-20260908/p23-final-full-gate.log`.

Only the new failed-run DerivedData was removed, restoring about 788 MB. The exact 702-path before/after manifest matches and `git diff --check` passes. The signed Release metadata/entitlements remain correct; a read-only `lsof` snapshot showed no active network listener or camera-related file for the running CXL process. Operator launch behavior and all hardware checks remain NOT RUN. Several gigabytes must be reclaimed before retrying the full Xcode gate; preserve earlier evidence unless cleanup is separately authorized.

Karl then explicitly authorized the reviewed cache cleanup. Xcode DerivedData, downloaded iOS DeviceSupport data, and 66 CXL build/object/cache directories were removed while all source worktrees, receipts, logs, result bundles, manifests, and the current signed P22c app were preserved. Free space rose to about 60 GB.

The frozen post-repair gate is now complete. Source digest `64f43b4d8f4334ef2d8f1be4b21be57923230f91d9a784c063ee2bb7bab52506` covers 702 paths and matches after verification. Python passed 82/82. Desktop passed 4,515 unique tests (4,460 passed/55 skipped/zero failed) and 9,302 runs across two configurations (9,192 passed/110 skipped/zero failed), with the two established calibration-export main-thread warnings. iOS and macOS builds passed. The wrapper rejected its target-only Watch invocation because Xcode does not accept `-derivedDataPath` without a scheme; a separate isolated Watch build passed. Preserved evidence is under `/private/tmp/scratchlab-cxl-auto-20260908/receipts/`, including `p23b-final-full-gate.xcresult` and its JSON summary.

Rebuild caches from this final gate were removed after its result/log evidence was retained. The current signed app remains unchanged. The next boundary is the existing first operator-visible Rane ONE diagnostic; no physical result is inferred from these passing software gates.


## 2026-09-08 — Operator permission report; preview enable pending

Karl reports that “all permissions” appeared earlier, then that the app reopened with no permission prompt and no camera preview. The specific prompts, triggering action, and granted/denied decisions remain unspecified. Do not mark the no-launch-permission check PASS from a subsequent quiet reopen. Blank preview is consistent with the explicit camera/audio enable boundary; camera readiness and physical Rane results remain NOT RUN.

Read-only follow-up confirmed ReferenceAuthoringView enters discovery with Serato disabled and explicit video selection required; camera/microphone requests are under the explicit engine start. Auxiliary Release services remain off. Companion receiver eagerly constructs MCSession/browser to assign delegates, but no evidence proves this causes the reported prompts; no source fix was made. The UI-control helper separately crashes with EXC_BREAKPOINT in Array.remove(at:); this is not a ScratchLab crash diagnosis.

Next operator action: in Setup, select a camera physically in the room and the exact Rane ONE MKII audio input, leave iPhone & Watch Relay off, then press Enable Selected Camera & Audio once. Confirm whether the selected local camera preview appears before proceeding to notation. Keep the previous permission burst unresolved until its timing/types can be identified. No source edits, builds, test runs, recording, or acceptance occurred in this follow-up.


## 2026-09-08 — Camera/audio enable recovered; live notation next

After reporting the generic “camera preview is not running / recording blocked” banner, Karl confirmed “ok all working now” in reply to the camera-selection/enable-button question. Record this as operator-confirmed recovery of camera/audio setup, not proof of a recorded take, notation correctness, signal routing, or production-rig acceptance. Agent logs for exact candidate PID 9724 show AVCaptureSessionDidStartRunningNotification at 21:19:30; no physical preview image was agent-observed because the UI-control helper remained unavailable. Executable SHA-256 and capture entitlements still match the signed P22c candidate.

No runtime root cause was established and no source fix or rebuild was made. Read-only investigation found that the visible inactive-camera banner is generic, the engine also stops camera preview if required audio attachment fails, and input creation currently discards underlying errors with try?. A selection change during start/discovery can also queue configuration before its explicit permission sequence; none of these paths was proven responsible for this operator report. The earlier broad permission burst remains unresolved.

Next: bounded live-preview AHHH loop test with Tear selected on Rane ONE MKII. Observe about ten seconds of right-platter playback and report whether the trace remains continuous across sample-loop boundaries. No recording or physical acceptance is inferred.


## 2026-09-08 — Relay handshake failing before Watch evidence reaches Mac

Karl reports relay not working, iPhone “Motion ready”, Watch “Device paired”. Current exact signed P22c app process is now PID 11815 (PID 9724 was from a previous launch). Preserved log receipts/relay-current-11815.log shows repeated ten-second iPhone invitation timeouts and transitions Not Connected -> Not Connected through 21:25:35. Framework calls these timed-out invitations declined; do not infer the user actively declined. No successful phone-to-Mac relay handshake is shown. The previous PID log receipts/relay-diagnostic-20260908.log is historical only.

Firewall is enabled and a read-only socketfilterfw check says incoming connections to the exact P22c bundle are permitted. No firewall or permission settings changed. CoreDevice sees the physical iPhone and Watch as available/paired; that is not app-relay acceptance. Current phone source confirms Motion ready is local Watch readiness or recent imported motion, independent of Mac connection, and invitation receipt is automatically accepted. The iPhone Watch Capture screen has a separate Mac ScratchLab Connected/Waiting badge. Installed phone binary version remains unverified.

Next bounded recovery: close and reopen ScratchLab on the iPhone, then keep Advanced -> Watch Capture visible and check whether Mac ScratchLab changes to Connected. No active Mac take was shown in operator screenshots. Do not restart the Mac or disrupt its working camera/audio for this first retry. A foreground phone retry and a new handshake/heartbeat observation are still needed. Camera/audio recovery remains operator-confirmed; the latest Tear screenshot shows a long take-local trace, but loop-boundary behavior and Tear accuracy still need dynamic operator confirmation. No source edits, builds, installs, recording, calibration, or acceptance were performed during relay diagnosis.


## 2026-09-08 — Corrected unreachable iPhone relay instructions

Karl correctly reports no Advanced -> Watch Capture. Source audit confirms the Watch Capture action is in unused layouts; active Advanced exposes Audio & DVS, MIDI & Controller, and Performer Monitor. The earlier guidance was wrong and is superseded. Current source capture-screen appearance starts companion advertising already. Installed phone app inspection confirms ScratchLab com.machelpnz.scratchlab 1.0.1 build 23, running PID 2949, with identity receipt ../receipts/iphone-scratchlab-installed.json; no install/update performed and installed source parity is not established. Next phone-only retry is relaunch ScratchLab and return to the same Motion ready capture screen; inspect current Mac relay state/logs. No source edit or claim that missing navigation caused invitation timeout.


## 2026-09-08 — Fresh phone retry still times out; iPhone Local Network permission pending

User replied “no” to relay recovery; interpreted explicitly as still disconnected. Agent restarted only installed iPhone com.machelpnz.scratchlab with devicectl --terminate-existing --console. Console is still attached in exec session 87999, writing ../receipts/iphone-relay-console.log; catchable signals to devicectl are forwarded to the app, so do not casually Ctrl-C this console while preserving the phone app. Mac PID 11815 was left running with camera/audio intact.

Phone console confirmed paired=true, installed=true and alternating reachable=true/false, proving at least intermittent phone-to-Watch reachability, not sustained relay or capture acceptance. Fresh Mac log ../receipts/relay-after-phone-restart.log shows a NEW phone peer identity after the restart but repeated ten-second invitation timeouts with no Connecting/Connected transition. Old log had POSIX 57 Socket is not connected while sending framework Hello. No app payload/version or encryption failure was established.

Mac read-only firewall checks: firewall enabled, exact P22c bundle incoming allowed, global block-all disabled. Agent also read System Settings Local Network via CUA: every displayed ScratchLab entry on, with no permission toggles changed. System Settings remains on that page. iPhone own saved localNetworkRationaleAccepted key is unset (receipt iphone-scratchlab-preferences.plist); this is only the in-app rationale flag, NOT OS permission denial; current capture-screen source starts advertising regardless of that flag.

Pending question to Karl: iPhone Settings -> Privacy & Security -> Local Network: is ScratchLab on? Do not confuse this with Mac settings. Source, build, install, app permissions, firewall, encryption and payload schemas unchanged. Missing Watch Capture navigation is a separate confirmed source issue; it is not the established transport cause. No physical recording/notation/relay acceptance.
