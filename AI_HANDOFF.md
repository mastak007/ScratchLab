...
EVIDENCE PROVENANCE. Take 007 user-observed screenshots exposed the authoring/review problem. The repair's deterministic verification uses synthetic fixtures; no physical artifact was parsed or modified in this slice. **No Take 008 artifact exists on this machine** - earlier drafts of these records said otherwise and were wrong. The RoutineCaptures store holds takes 001-007 only and was byte-identical throughout (202 files). Scope is **15 implementation/test files plus four workflow documents**, not the "19 files" earlier drafts claimed.
