...
**Second camera orientation** lets you choose Portrait (90°) or Portrait (270°) if turning the iPhone does not change its feed. Choose the option that makes the preview upright. The same supported rotation is applied to preview and recording, remembered for that camera, and locked during a take. Automatic uses the camera’s reported orientation.

**Change cameras for next take** in Review returns to the camera pickers without discarding the saved take. Both pickers remain available between recordings. A main-camera change briefly disables Record while the new input connects; **Reconnect selected cameras** retries an enabled setup. If activation failed or the preview is stopped, choose another camera and use **Enable Selected Camera & Audio**. Camera changes are locked during count-in, capture, finalization and export. Changes affect future recordings only; use New scratch first when returning from a saved draft.
...
If the Mac stays at **Searching for companion device**, use **Reconnect iPhone relay** in CXL and **Reconnect Mac** in the iPhone companion. If more than one device appears, use **Connect to [device]**. Read the separate Mac connection status; the Watch being paired or connected to the phone does not establish a Mac relay. Retry outside an active recording.
...
For review now, choose **Review bounds** on a repetition. A compact recorded video appears beside its Start/End beat controls, using the same audio and camera views as whole-take playback. **Show start** and **Show end** pause at the selected edge. **Play repetition** auditions only those bounds; **Play with rest context** includes up to four beats before and after, limited by the recorded media. This can include a neighbouring scratch and does not change the saved bounds or replace the rest bars in the recording. Adjusting a bound pauses playback and shows that edge. Mark the best repetition after listening.

**Approve & Save Capture…**, directly below the four repetitions and again at the end of review, runs the canonical checks, saves the approved draft on this Mac, then opens the ZIP save dialog. Cancelling or failing the ZIP save leaves the approved draft intact; use the nearby **Save Capture…** to retry. **Approve Canonical Draft Only** remains available if export should wait. A raw ZIP retains the complete recordings and selected-repetition metadata; it does not replace the separate approved reference package.
